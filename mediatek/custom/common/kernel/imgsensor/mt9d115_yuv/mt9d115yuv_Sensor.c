 ...
 * .
 * Fix MT9D115 YUV sensor frame rate to 30fps in vidoe mode
 *
 * Mar 4 2010 mtk70508
...
/*************************************************************************
* FUNCTION
*    MT9D115_burst_write_XDMA
*
* DESCRIPTION
*    This function burst wirte XDMA to CMOS sensor through I2C
*    MT9D115_MAX_I2C_FAIL_TRY_NO is the max try number. when write fail, the function retry again until
*    try times reach MT9D115_MAX_I2C_FAIL_TRY_NO.
*
* PARAMETERS
...
*
* DESCRIPTION
*    This function Patch the MT9D115 which provided by Micron
*
* PARAMETERS
*    None
*
...
/*************************************************************************
* FUNCTION
*    MT9D115_set_cap_dummy
*
* DESCRIPTION
*    This function add dummy_pixel and dummy_line in capture state.
*    MUST BE INVOKED BEFORE MT9D115_set_cmd(2) !!!
*
* PARAMETERS
*    dummy_pixel
*    dummy_line
...
*    MUST BE INVOKED AFTER MT9D115_preview() !!!
*
* PARAMETERS
*    None
*
...
/*	if((image_window->ImageTargetWidth <= MT9D115_IMAGE_SENSOR_PV_WIDTH )&&
		image_window->ImageTargetHeight <= MT9D115_IMAGE_SENSOR_PV_HEIGHT)
	{
		image_window->GrabStartX = iStartX + 8;
		image_window->GrabStartY = iStartY +7;
		image_window->ExposureWindowWidth = MT9D115_IMAGE_SENSOR_PV_WIDTH - 8;
		image_window->ExposureWindowHeight = MT9D115_IMAGE_SENSOR_PV_HEIGHT - 6;
	}
	else
	{
		...
		iDummy_pixel = 0x02;
		if(MT9D115_zoom_factor >= 3 && MT9D115_zoom_factor <= 6)
			iDummy_pixel = 0x4E4;
		else if (MT9D115_zoom_factor > 6)
			iDummy_pixel = 0x7D2;
		
		MT9D115_Config_Cap_Blank(iDummy_pixel,0x00);//MT9D115_Config_Cap_Blank(iDummy_pixel,0x3D);
		//use group_A register
		MT9D115_set_page(0);
		MT9D115_Control = MT9D115_Control |0x10; 
		MT9D115_write_cmos_sensor(0x04,MT9D115_Control);
		Sleep(30);

	
		image_window->GrabStartX = iStartX + 8;
		image_window->GrabStartY = iStartY + 7;
		image_window->ExposureWindowWidth = MT9D115_IMAGE_SENSOR_FULL_WIDTH - 8;
		image_window->ExposureWindowHeight = MT9D115_IMAGE_SENSOR_FULL_HEIGHT - 6;
	}
		// copy sensor_config_data
		memcpy(&MT9D115SensorConfigData, sensor_config_data, sizeof(MSDK_SENSOR_CONFIG_STRUCT));
		*/
		return ERROR_NONE;
