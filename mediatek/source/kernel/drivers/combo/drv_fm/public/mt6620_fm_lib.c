...
int fm_bop_write(uint8_t addr, uint16_t value, unsigned char *buf, int size)
{
    ...
    WCN_DBG(FM_DEBUG|D_MAIN,"%02x %02x %02x %02x %02x \n", buf[0], buf[1], buf[2], buf[3], buf[4]);
...
int fm_bop_udelay(uint32_t value, unsigned char *buf, int size)
{
    ...
    WCN_DBG(FM_DEBUG|D_MAIN,"%02x %02x %02x %02x %02x %02x \n", buf[0], buf[1], buf[2], buf[3], buf[4], buf[5]);
...
int fm_bop_rd_until(uint8_t addr, uint16_t mask, uint16_t value, unsigned char *buf, int size)
{
    ...
    WCN_DBG(FM_DEBUG|D_MAIN,"%02x %02x %02x %02x %02x %02x %02x \n", buf[0], buf[1], buf[2], buf[3], buf[4], buf[5], buf[6]);
...
int mt6620_tune_txscan(unsigned char *tx_buf, int tx_buf_size, uint16_t freq)
{
	...
	pkt_size += fm_bop_rd_until(0x6F, 0x0001, 0x0001, &tx_buf[pkt_size], tx_buf_size - pkt_size);//Poll fm_intr_stc_done (69H D0) = 1
...
// TBD for IC
int mt6620_seek(unsigned char *tx_buf, int tx_buf_size, uint16_t seekdir, uint16_t space, uint16_t max_freq, uint16_t min_freq)
{
    int pkt_size = 0;
...
#if 0
    //A4 Wait for STC_DONE interrupt, do in FM firmware
    //A5 Check STC_DONE interrupt status flag
    pkt_size += fm_bop_rd_until(0x69, 0x0001, 0x0001, &tx_buf[pkt_size], tx_buf_size - pkt_size);//Poll fm_intr_stc_done (69H D0) = 1
    //A6 Write 1 clear the STC_DONE interrupt status flag
    pkt_size += fm_bop_write(0x69, 0x0001, &tx_buf[pkt_size], tx_buf_size - pkt_size);//wr 69 1

    //A7 A8, set freq to new, if APP will tune to the new channel, we need do nothing here
...
int mt6620_scan(unsigned char *tx_buf, int tx_buf_size, uint16_t scandir, uint16_t space, uint16_t max_freq, uint16_t min_freq)
{
    int pkt_size = 0;
