...
/*
   Suspend_reg.Suspend_VAUDN_CON0 = Ana_Get_Reg(VAUDN_CON0);
   Suspend_reg.Suspend_VAUDP_CON0 = Ana_Get_Reg(VAUDP_CON0);
   Suspend_reg.Suspend_VAUDP_CON1 = Ana_Get_Reg(VAUDP_CON1);
   Suspend_reg.Suspend_VAUDP_CON2 = Ana_Get_Reg(VAUDP_CON2);
   Suspend_reg.Suspend_VA12_CON0 = Ana_Get_Reg(VA12_CON0);
   Suspend_reg.Suspend_VMIC_CON0 = Ana_Get_Reg(VMIC_CON0);
   Suspend_reg.Suspend_VA25_CON0 = Ana_Get_Reg(VA25_CON0);
*/
...
/*
   // Clean used buffer (HW already consume)
   if((Afe_Block->u4DMAReadIdx + Afe_consumed_bytes) < (Afe_Block->u4BufferSize))
   {
      memset(Afe_Block->pucVirtBufAddr + Afe_Block->u4DMAReadIdx , 0 , Afe_consumed_bytes);
   }
   else
   {
      if(Afe_Block->u4BufferSize > 0){
         kal_uint32 size_1 = Afe_Block->u4BufferSize - Afe_Block->u4DMAReadIdx;
         kal_uint32 size_2 = Afe_consumed_bytes - size_1;
         memset(Afe_Block->pucVirtBufAddr + Afe_Block->u4DMAReadIdx , 0 , size_1);
         memset(Afe_Block->pucVirtBufAddr  , 0 , size_2);
      }
   }
*/
...
// Use SysRam (only for Test)
/*
   printk("AudDrv_Init_I2S_InputStream MT6573_SYSRAM_ALLOC \n");
   ...
   if(NULL == I2S_input_Control->rBlock.pucPhysBufAddr){
      printk("AudDrv_Init_I2S_InputStream MT6573_SYSRAM_ALLOC fail \n");
      return;
   }
 ...
 *
 * PARAMETERS
 *  period  [out]  return time in jiffices
 *
 * RETURNS
 *  None
 *
