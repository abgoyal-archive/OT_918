...
static void tmd2771_eint_work(struct work_struct *work)
{
	...
	if((err = tmd2771_check_and_clear_intr(obj->client)))
	{
...
int tmd2771_setup_eint(struct i2c_client *client)
{
	...
	MT6516_EINT_Registration(CUST_EINT_ALS_NUM, CUST_EINT_ALS_DEBOUNCE_EN, CUST_EINT_ALS_POLARITY, tmd2771_eint_func, 0);
...
/*
	if(err = tmd2771_setup_eint(client))
	{
		APS_ERR("setup eint: %d\n", err);
		return err;
	}
	if(err = tmd2771_check_and_clear_intr(client))
	{
