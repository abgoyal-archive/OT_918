 ...
 * Description:
 * ------------
 *   This Module defines functions of 6573 fuel gauge
 *
 * Author:
 * -------
 * -------
...
kal_int32 fgauge_read_columb(void)
{
	...
	if(R_FG_VALUE != 20)
	{
		...
		dvalue_CAR = (dvalue_CAR*R_FG_VALUE)/20;
		if (Enable_FGADC_LOG == 1) {
			printk("new CAR=%d\n", dvalue_CAR);			
		}
		...
		dvalue_NTER = (dvalue_NTER*R_FG_VALUE)/20;
		if (Enable_FGADC_LOG == 1) {
			printk("new NTER=%d\n", dvalue_NTER);			
		}
	}
