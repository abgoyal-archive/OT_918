...
static int MT6620_GetCurRSSI(uint16_t *pRSSI);
                                ...
                                int timeout)
{
    ...
    if((NULL == tx_buf) || (pkt_size < 0) || (0 == flag_mask) 
        ...
        WCN_DBG(FM_ERR|D_ALL,"%s():invalid para\n", __func__);
        ret_val = -ERR_INVALID_PARA;
        goto out;
    }
     WCN_DBG(FM_DEBUG|D_MAIN,"0,flag_mask=0x%08x\n", flag_mask);
sw_retry:
     //WCN_DBG(FM_DEBUG|D_MAIN,"0,flag_mask=0x%08x\n", flag_mask);
    if(mtk_wcn_stp_send_data(tx_buf, pkt_size, FM_TASK_INDX) == 0){
        WCN_DBG(FM_EMERG|D_MAIN,"%s():send data over stp failed\n", __func__);
        ret_val = -ERR_STP; //stp error is the most serious error, wo can do nothing but info APP
        goto out;
    }else{
        //WCN_DBG(FM_DEBUG|D_MAIN,"1,flag_mask=0x%08x\n", flag_mask);
        ret_time = wait_event_timeout(fm_wq, ((flag & flag_mask) == flag_mask), timeout*HZ);
        WCN_DBG(FM_DEBUG|D_MAIN,"[flag=0x%08x]\n", flag);
        if(!ret_time){
           if(0 < retry_cnt--){
              WCN_DBG(FM_WARNING|D_MAIN,"%s():wait even timeout, [retry_cnt=%d]\n", __func__, retry_cnt);
              goto sw_retry;
           }else{
              WCN_DBG(FM_ALERT|D_MAIN,"%s():fatal error, SW retry failed, reset HW\n", __func__);
              ret_val = -ERR_FW_NORES; //return fatal error
              goto out;
           }
        }
        WCN_DBG(FM_DEBUG|D_MAIN,"2,flag_mask=0x%08x\n", flag_mask);
        flag &= ~flag_mask;
    }
out:
    return ret_val;
...
int MT6620_read(uint8_t addr, uint16_t *val)
{
    ...
    WCN_DBG(FM_DEBUG|D_CMD,"+%s(): [addr=0x%02x]\n", __func__, addr);
    WCN_DBG(FM_INFO|D_CMD,"-fm_cmd_mutex\n");
    if (down_interruptible(&fm_cmd_mutex)){
        WCN_DBG(FM_CRIT|D_CMD|D_ALL, "%s(): get mutex failed\n", __func__);
        ret_val = -ERR_GET_MUTEX;
        goto out1;
    }
    
	if(tx_buf == NULL){
        WCN_DBG(FM_ERR|D_CMD|D_ALL, "%s(): invalid tx_buf\n", __func__);
...
out1:
    WCN_DBG(FM_DEBUG|D_CMD,"-%s(): [val=0x%02x]\n", __func__, fspi_rd);
    return ret_val;
}
...
int MT6620_set_bits(uint8_t addr, uint16_t bits, uint16_t mask)
{
    ...
    WCN_DBG(FM_DEBUG|D_CMD,"+%s(): [addr=0x%02x],[bits=0x%04x],[mask=0x%04x]\n", __func__, addr, bits, mask);
...
out:
    WCN_DBG(FM_DEBUG|D_CMD,"-%s():[ret=%d]\n", __func__, ret);
    return ret;
}
...
static int MT6620_Mute(bool mute)
{
    ...
    WCN_DBG(FM_DEBUG|D_CMD,"+%s():[mute=%d]\n", __func__, mute);
...
static int MT6620_SetFreq(uint16_t freq)
{
    ...
    WCN_DBG(FM_DEBUG|D_CMD,"+%s():[freq=%d]\n", __func__, freq);
    WCN_DBG(FM_INFO|D_CMD,"-fm_cmd_mutex\n");
    if (down_interruptible(&fm_cmd_mutex)){
        WCN_DBG(FM_CRIT|D_CMD|D_ALL,"%s(): get mutex failed\n", __func__);
        ret = -ERR_GET_MUTEX;
    ...
    WCN_DBG(FM_NOTICE|D_CMD,"tune[freq=%d]\n", freq);
#ifdef CTP_CHECK_FM
...
static int MT6620_Seek(uint16_t min_freq, uint16_t max_freq, uint16_t *pFreq, uint16_t seekdir, uint16_t space)
{
    ...
    WCN_DBG(FM_DEBUG|D_CMD,"+%s():[startfreq=%d]\n", __func__, startfreq);

    if((ret = MT6620_RampDown())){
        WCN_DBG(FM_ALERT|D_CMD|D_ALL,"%s():RampDown failed\n", __func__);
        goto out1;
    }

    if ((ret = MT6620_SetFreq(startfreq))){
         WCN_DBG(FM_ALERT|D_CMD|D_ALL,"%s():set freq failed\n", __func__);
         goto out1;
    }
    ...
    pkt_size = mt6620_seek(tx_buf, TX_BUF_SIZE, seekdir, space, max_freq, min_freq);
    ret = fm_send_wait_timeout(tx_buf, pkt_size, 
                                    FLAG_SEEK | FLAG_SEEK_DONE, SW_RETRY_CNT, SEEK_TIMEOUT);
    if(!ret){
        *pFreq = seek_result;
    _current_frequency = seek_result;
    WCN_DBG(FM_NOTICE|D_CMD,"seek[freq=%d]\n", seek_result);
        Delayms(35);
#ifdef CTP_CHECK_FM
        tpd_get_fm_frequency(_current_frequency);
#endif
    }else{
        WCN_DBG(FM_WARNING|D_CMD,"Rx Seek Failed\n");
    }
...
static int MT6620_TxScan_GetCQI(int16_t *pRSSI, int16_t *pPAMD, int16_t *pMR)
{
	...
	if((pRSSI == NULL) || (pPAMD == NULL) || (pMR == NULL)){
        WCN_DBG(FM_ERR|D_CMD|D_ALL,"%s():para error, [pRSSI=%p],[aPAMD=%p],[pMR=%p]\n", 
	...
	*pMR = aMR>>3;
    ...
    WCN_DBG(FM_DEBUG|D_CMD,"%s():[RSSI=%d],[PAMD=%d],[MR=%d]\n", 
            __func__, *pRSSI, *pPAMD, *pMR);
...
static int MT6620_TxScan_IsEmptyChannel(int16_t RSSI, int16_t PAMD, int16_t MR, int *empty)
{
    ...
    if(empty == NULL){
        WCN_DBG(FM_DEBUG|D_CMD,"invalid pointer [empty=0x%x]!\n", (unsigned int)empty);
        ret = -ERR_INVALID_PARA;
        goto out;
    }
                        ...
                        uint16_t space)
{
    ...
    if((!pScanTBL) || (*ScanTBLsize < TX_SCAN_MIN) || (*ScanTBLsize >  TX_SCAN_MAX)){
        WCN_DBG(FM_ERR|D_CMD|D_ALL,"+%s():invalid scan table\n", __func__);
        ret = -ERR_INVALID_PARA;
        goto out;
    }

    WCN_DBG(FM_DEBUG|D_CMD, "[freq=%d], [max_freq=%d],[min_freq=%d],[scan BTL size=%d],[scandir=%d],[space=%d]\n", 
	...
	while(!(!(cnt < scan_cnt) || (freq > max_freq) || (freq < min_freq))){
		...
		if(ret){
            WCN_DBG(FM_CRIT|D_CMD|D_ALL,"%s():set freq failed\n", __func__);
			goto out;
		}
		...
		if(ret){
            WCN_DBG(FM_CRIT|D_CMD|D_ALL,"%s():get CQI failed\n", __func__);
			goto out;
		}
		...
		if(!ret){
            if((empty == TRUE) && ((freq < FMTX_SCAN_HOLE_LOW) || (freq > FMTX_SCAN_HOLE_HIGH))){
                *(pScanTBL + cnt) = freq; //strore the valid empty channel 
			    cnt++;
                WCN_DBG(FM_NOTICE|D_CMD|D_ALL,"empty channel:[freq=%d] [cnt=%d]\n", freq, cnt);
            }			
		}else{ 
		    WCN_DBG(FM_CRIT|D_CMD|D_ALL,"%s():IsEmptyChannel failed\n", __func__);
			goto out;
        }
		...
		if(scandir == SCAN_UP){
            ...
            WCN_DBG(FM_ERR|D_CMD|D_ALL,"%s():scandir para error\n", __func__);
			ret = -ERR_INVALID_PARA;
            goto out;
		}
	}
	...
	*ScanTBLsize = cnt;
	*pFreq = *(pScanTBL + cnt);
	WCN_DBG(FM_DEBUG|D_CMD, "completed, [cnt=%d],[freq=%d],[counter=%d]\n", cnt, freq, counter);
    WCN_DBG(FM_INFO|D_CMD, "--time end[%d]\n", jiffies_to_msecs(jiffies));
                        ...
                        uint16_t space)
{
    ...
    if((!pScanTBL) || (tmp_scanTBLsize < MT6620_SCANTBL_SIZE)){
        WCN_DBG(FM_ERR|D_ALL,"+%s():invalid scan table\n", __func__);
        ret = -ERR_INVALID_PARA;
        goto out1;
    }

    WCN_DBG(FM_INFO|D_CMD, "[freq=%d], [max_freq=%d],[min_freq=%d],[scan BTL size=%d],[scandir=%d],[space=%d]\n", 
                *pFreq, max_freq, min_freq, *ScanTBLsize, scandir, space);
...
static int MT6620_ScanForceStop()
{
    ...
    WCN_DBG(FM_DEBUG|D_CMD,"[flag=0x%08x]\n", flag);
    wake_up(&fm_wq);
...
static int MT6620_GetCurRSSI(uint16_t *pRSSI)
{
    ...
    if((ret = MT6620_read(FM_ADDR_RSSI, &tmp_reg)))
        goto out;
    WCN_DBG(FM_DEBUG|D_CMD,"[addr=0x%02x],[val=0x%04x]\n",\
    ...
    WCN_DBG(FM_DEBUG|D_CMD,"[RSSI=%d],[dBuv=%d]\n",\
			rssi, *pRSSI);
out:
    WCN_DBG(FM_DEBUG|D_CMD,"-%s():[ret=%d]\n", __func__, ret);
    return ret;
...
static int MT6620_I2S_Setting(int onoff, int mode, int sample)
{
    ...
    if(onoff == FM_I2S_ON){
        ...
        WCN_DBG(FM_ERR|D_CMD|D_ALL,"%s():[onoff=%d]\n", __func__, onoff);
        ret = -ERR_INVALID_PARA;
        goto out;
    }
    ...
    if(mode == FM_I2S_MASTER){
        ...
        WCN_DBG(FM_ERR|D_CMD|D_ALL,"%s():[mode=%d]\n", __func__, mode);
        ret = -ERR_INVALID_PARA;
        goto out;
    }
    ...
    if(sample == FM_I2S_32K){
        ...
        WCN_DBG(FM_ERR|D_CMD|D_ALL,"%s():[sample=%d]\n", __func__, sample);
        ret = -ERR_INVALID_PARA;
        goto out;
    }
    ...
    WCN_DBG(FM_NOTICE|D_CMD,"[onoff=%s][mode=%s][sample=%d](0)33KHz,(1)44.1KHz,(2)48KHz\n", 
...
static int MT6620_FMOverBT(int enable)
{
    ...
    if(inited == false){
        ...
        if((ret = MT6620_read(0x5F, &sample)))
            goto out;
        inited = true;
        WCN_DBG(FM_NOTICE|D_MAIN,"init, record priv seetings\n");
    }
    ...
    if(enable == TRUE){
        ...
        if((ret = MT6620_write(0x5F, 0x1175)))
            goto out;
        WCN_DBG(FM_NOTICE|D_MAIN,"set FM via BT controller\n");
        ...
        if((ret = MT6620_write(0x5F, sample)))
            goto out;
        WCN_DBG(FM_NOTICE|D_MAIN,"set FM via Host\n");
    }else{
        WCN_DBG(FM_ERR|D_CMD|D_ALL,"%s()\n", __func__);
        ret = -ERR_INVALID_PARA;
        goto out;
    }
out:
...
static int MT6620_ANA_SWITCH(int antenna)
{
	...
	WCN_DBG(FM_DEBUG|D_CMD,"+%s():\n", __func__);
    WCN_DBG(FM_NOTICE|D_CMD,"Switch to %s atenna\n", (antenna == FM_LONG_ANA) ? "Long" : "Short");
...
static int MT6620_dump_reg(void)
{
    ...
    for (i = 0; i < MT6620_SCANTBL_SIZE; i++){
        ...
        if (ret == 0){
            WCN_DBG(FM_DEBUG|D_CMD,"%2d\t%04x\n", i, val);
        }else{
            WCN_DBG(FM_DEBUG|D_CMD|D_ALL,"%2d\tXXXX\n", i);
            goto out;
        }
    }
...
static int MT6620_GetMonoStereo(uint16_t *pMonoStereo)
{
    ...
    *pMonoStereo = tmp_reg;//1:stereo, 0:mono
    WCN_DBG(FM_DEBUG|D_CMD,"[MonoStereo=%d]\n", *pMonoStereo);
...
static int MT6620_GetCapArray(int *caparray)
{
    ...
    *caparray = (int)tmp_reg;
    WCN_DBG(FM_DEBUG|D_CMD,"[caparray=%d]\n", *caparray);
...
out:
    WCN_DBG(FM_DEBUG|D_CMD,"[PamdLevl=%d]\n", (int)*pPamdLevl);
    return ret;
}
...
static int MT6620_em_test(uint16_t group_idx, uint16_t item_idx, uint32_t item_value)
{
    ...
    WCN_DBG(FM_DEBUG|D_CMD,"+%s():[group_idx=%d],[item_idx=%d],[item_value=%d]\n", 
...
static int MT6620_Rds_Tx(uint16_t pi, uint16_t *ps, uint16_t *other_rds, uint8_t other_rds_cnt)
{
    ...
    WCN_DBG(FM_DEBUG|D_CMD,"+%s():PI=0x%04x, PS=0x%04x/0x%04x/0x%04x, other_rds_cnt=%d \n", 
...
static int fm_enable_rds_BlerCheck(struct fm *fm)
{
    ...
    if(ret){
        WCN_DBG(FM_ALERT|D_MAIN,"down fm_timer_mutex failed[ret=%d]\n", ret);
        goto out;
    }
    timer_sys.rds_reset_en = FM_RDS_RST_ENABLE;
    WCN_DBG(FM_DEBUG|D_TIMER,"enable RDS reset func\n");
    up(&fm_timer_mutex);
...
static int fm_disable_rds_BlerCheck()
{
    ...
    WCN_DBG(FM_DEBUG|D_TIMER,"disable RDS reset func\n");
	up(&fm_timer_mutex);
...
static int fm_get_gps_rtc_info(struct fm_gps_rtc_info *dst, struct fm_gps_rtc_info *src)
{
    ...
    if(src->retryCnt > 0){
        ...
        WCN_DBG(FM_DEBUG|D_MAIN,"%s, new [retryCnt=%d]\n", __func__, dst->retryCnt);
    }
    if(src->ageThd > 0){
        dst->ageThd = src->ageThd;
        WCN_DBG(FM_DEBUG|D_MAIN,"%s, new [ageThd=%d]\n", __func__, dst->ageThd);
    }
    if(src->driftThd > 0){
        dst->driftThd = src->driftThd;
        WCN_DBG(FM_DEBUG|D_MAIN,"%s, new [driftThd=%d]\n", __func__, dst->driftThd);
    }
    if(src->tvThd.tv_sec > 0){
        dst->tvThd.tv_sec = src->tvThd.tv_sec;
        WCN_DBG(FM_DEBUG|D_MAIN,"%s, new [tvThd=%d]\n", __func__, (int)dst->tvThd.tv_sec);
    }
    
    while(down_trylock(&fm_rtc_mutex)){
        WCN_DBG(FM_WARNING|D_MAIN,"down_trylock failed\n");
        if(++retry_cnt < dst->retryCnt){
            WCN_DBG(FM_WARNING|D_MAIN,"[retryCnt=%d]\n", retry_cnt);
            msleep_interruptible(50); 
            continue;
        }else{
            WCN_DBG(FM_WARNING|D_MAIN,"down_trylock retry failed\n");
            ret = -EFAULT;
            goto out;
        }    
    }
   ...
   /*
    ...
    if(timer_sys.tx_rtc_ctrl_en == FM_TX_RTC_CTRL_ENABLE){
        WCN_DBG(FM_DEBUG|D_TIMER,"fm_tx_rtc_ctrl_work, ticks:%d\n", jiffies_to_msecs(jiffies));
...
{
    int ret = 0;
    WCN_DBG(FM_DEBUG|D_MAIN,"+%s():\n", __func__);

    //check if we are resetting
    if(subsys_rst_state != FM_SUBSYS_RST_OFF){
        WCN_DBG(FM_NOTICE|D_MAIN, "FM subsys resetting is ongoing!!!\n");
        goto out;
    }
    ...
    if (MTK_WCN_BOOL_FALSE == mtk_wcn_wmt_func_off(WMTDRV_TYPE_FM)){
        WCN_DBG(FM_ALERT|D_MAIN,"WMT turn off FM fail!\n");
        ret = -ERR_STP;
    }else{
        WCN_DBG(FM_NOTICE|D_MAIN,"WMT turn off FM OK!\n");
    }
...
out:
    WCN_DBG(FM_DEBUG|D_MAIN,"-%s():[ret=%d]\n", __func__, ret);
    return ret;
}
...
static void fm_timer_func(unsigned long data)
{
    ...
    if((timer->tx_pwr_ctrl_en == FM_TX_PWR_CTRL_ENABLE) && (timer->count%g_fm_config.txpwrctl == 0)){
        WCN_DBG(FM_DEBUG|D_TIMER,"fm_tx_power_ctrl_work, ticks:%d\n", jiffies_to_msecs(jiffies));
    ...
    if(g_fm_config.vcooff < 1){
        WCN_DBG(FM_WARNING|D_TIMER,"vco tracking time err\n");
    ...
    if((timer->tx_desense_en == FM_TX_DESENSE_ENABLE) && (timer->count%vco_cycle == 0)){
        WCN_DBG(FM_DEBUG|D_TIMER,"fm_tx_desense_wifi_work, ticks:%d\n", jiffies_to_msecs(jiffies));
    ...
    if(g_fm_config.timer < 1000){
        WCN_DBG(FM_WARNING|D_TIMER,"timersys time err\n");
        g_fm_config.timer = FM_TIMER_TIMEOUT_MIN;
    }
	mod_timer(&fm_timer, jiffies + g_fm_config.timer/(1000/HZ)); 
    WCN_DBG(FM_DEBUG|D_TIMER,"-T:%d,mod timer\n", jiffies_to_msecs(jiffies));

out:
    //WCN_DBG(FM_DEBUG|D_TIMER,"-%s():[onoff=%d]\n", __func__, timer->onoff);
    return;
}
static void fm_tx_rtc_ctrl_worker_func(struct work_struct *work)
{
    ...
    if(ret){
        WCN_DBG(FM_ALERT|D_MAIN,"down fm_rtc_mutex failed[ret=%d]\n", ret);
        goto out;
    }
    ...
    if(gps_rtc_info.flag == FM_GPS_RTC_INFO_NEW){
        ...
        WCN_DBG(FM_DEBUG|D_MAIN,"there's no new rtc drift info\n");
        up(&fm_rtc_mutex);
        goto out;
    }

    if(rtcInfo.age > rtcInfo.ageThd){
       WCN_DBG(FM_WARNING|D_MAIN,"age over it's threshlod\n");
       goto out;
    }
    if((rtcInfo.drift <= rtcInfo.driftThd) && (rtcInfo.drift >= -rtcInfo.driftThd)){
       WCN_DBG(FM_WARNING|D_MAIN,"drift over it's MIN threshlod\n");
       goto out;
    }

    if(rtcInfo.drift > FM_GPS_RTC_DRIFT_MAX){
        WCN_DBG(FM_WARNING|D_MAIN,"drift over it's +MAX threshlod\n");
    ...
    if((curTime.tv_sec - rtcInfo.tv.tv_sec) > rtcInfo.tvThd.tv_sec){
        WCN_DBG(FM_WARNING|D_MAIN,"time diff over it's threshlod\n");
        goto out;
    }*/
    curTime = jiffies;
...
static void fm_tx_power_ctrl_worker_func(struct work_struct *work)
{
    ...
    if(down_interruptible(&fm_rxtx_mutex)){
        WCN_DBG(FM_CRIT|D_TIMER, "%s(): get rx/tx mutex failed\n", __func__);
        return;
    }
    
    if(!fm){
        WCN_DBG(FM_ALERT|D_TIMER,"err, [fm=0x%08x]\n", (int)fm);
        goto out;
    }

    if(fm->powerup != FM_PWR_TX_ON){
        WCN_DBG(FM_NOTICE|D_TIMER,"FM is not on TX mode\n");
        goto out;
    }
    
    if(priv.state == INITED){
        WCN_DBG(FM_INFO|D_TIMER,"%s, tx_power_ctrl[0x%08x]\n", __func__, (unsigned int)priv.priv_tbl.tx_pwr_ctrl);
...
static void fm_rds_reset_worker_func(struct work_struct *work)
{
    ...
    if(fm->powerup != FM_PWR_RX_ON){
        WCN_DBG(FM_NOTICE|D_TIMER,"FM is not on RX mode\n");
        goto out;
    }
    
    WCN_DBG(FM_INFO|D_TIMER,"%s():-fm_read_mutex\n", __func__);
    if (down_interruptible(&fm_read_mutex)){
        WCN_DBG(FM_ALERT|D_ALL,"fm_rds_reset_worker_func can't get mutex");
        goto out;      
    }
    
    ret = MT6620_RDS_BlerCheck(fm);
    if(-ERR_FW_NORES == ret){
        WCN_DBG(FM_ALERT|D_TIMER|D_ALL,"fw no response, do hw reset\n");
        fm_hw_reset();
    }
    up(&fm_read_mutex); 
    WCN_DBG(FM_INFO|D_TIMER,"%s():+fm_read_mutex\n", __func__);
...
void stp_rx_event_cb(void)
{
    ...
    WCN_DBG(FM_DEBUG|D_MAIN,"cnt1=%d\n", ++cnt1);
    WCN_DBG(FM_DEBUG|D_MAIN,"%s()\n", __func__);  
}
    ...
    if(sz <= sizeof(ENUM_WMTRSTMSG_TYPE_T)){ 
        ...
        if((src==WMTDRV_TYPE_WMT) && (dst == WMTDRV_TYPE_FM) && (type == WMTMSG_TYPE_RESET)){                
            if(rst_msg == WMTRSTMSG_RESET_START){
                ...
                ret = down_interruptible(&fm_timer_mutex);
                if(ret){
                    WCN_DBG(FM_WARNING|D_MAIN, "down fm_timer_mutex failed!\n");
                ...
                if(ret){
                    WCN_DBG(FM_WARNING|D_MAIN, "down fm_ops_mutex failed!\n");
                    //FIX ME, how to handle this case?
                    return;
                }else{
                    fm_cb->powerup = FM_PWR_OFF;
        ...
        WCN_DBG(FM_WARNING|D_MAIN, "message format invalid!\n");
    }
}
...
static int fm_recover_func(struct fm *fm, struct fm_context *cxt)
{
    ...
    WCN_DBG(FM_NOTICE|D_MAIN,"+%s():\n", __func__);
    if(cxt->ref < 1){
        WCN_DBG(FM_NOTICE|D_MAIN,"FM not in use, no need recover\n");
        goto out;
    }else{
        //FM func on
        if (MTK_WCN_BOOL_FALSE == mtk_wcn_wmt_func_on(WMTDRV_TYPE_FM)){
            WCN_DBG(FM_WARNING|D_MAIN, "FM mtk_wcn_wmt_func_on failed!\n");
            //FIX ME, how to handle this case?
            goto out;
        }
        ...
        if(FALSE == mtk_wcn_stp_is_ready()){
            WCN_DBG(FM_ALERT|D_ALL,"6620 stp is not ready, please retry later\n");
            goto out;
        }
        ...
        switch(cxt->powerup){
            case FM_PWR_RX_ON:
                ...
                WCN_DBG(FM_NOTICE|D_MAIN, "[band=%d][freq=%d]\n", parm.band, parm.freq);
                if(ret){
                    WCN_DBG(FM_WARNING|D_MAIN, "fm_powerup failed!\n");
                    //FIX ME, how to handle this case?
                    goto out;
                }
                ...
                if(ret){
                    WCN_DBG(FM_WARNING|D_MAIN, "fm_setvol failed!\n");
                    //FIX ME, how to handle this case?
                    goto out;
                }*/
                //set mute if need
                WCN_DBG(FM_NOTICE|D_MAIN, "[rxcxt.mute=%d]\n", cxt->rxcxt.mute);
                ret = MT6620_Mute(cxt->rxcxt.mute);
                if(ret){
                    WCN_DBG(FM_WARNING|D_MAIN, "MT6620_Mute failed!\n");
                    //FIX ME, how to handle this case?
                    goto out;
                }

                //set FM antenna type
                WCN_DBG(FM_NOTICE|D_MAIN, "[rxcxt.ana=%d]\n", cxt->rxcxt.ana);
                ret = fm_antenna_switch(fm, cxt->rxcxt.ana);
                if(ret){
                    WCN_DBG(FM_WARNING|D_MAIN, "fm_antenna_switch failed!\n");
                    //FIX ME, how to handle this case?
                    goto out;
                }

                //set FM over bt
                WCN_DBG(FM_NOTICE|D_MAIN, "[rxcxt.ViaBt=%d]\n", cxt->rxcxt.ViaBt);
                ret = fm_over_bt(fm, cxt->rxcxt.ViaBt);
                if(ret){
                    WCN_DBG(FM_WARNING|D_MAIN, "fm_over_bt failed!\n");
                    //FIX ME, how to handle this case?
                    goto out;
                }

                //set FM RDS on/off
                WCN_DBG(FM_NOTICE|D_MAIN, "[rxcxt.rdsOn=%d]\n", cxt->rxcxt.rdsOn);
                ...
                if(ret){
                    WCN_DBG(FM_WARNING|D_MAIN, "MT6620_RDS_OnOff failed!\n");
                    //FIX ME, how to handle this case?
                    goto out;
                }
            ...
            case FM_PWR_TX_ON:
                ...
                if(ret){
                    WCN_DBG(FM_WARNING|D_MAIN, "fm_powerup_tx failed!\n");
                    //FIX ME, how to handle this case?
                    goto out;
                }
                 //set audio path to I2S
                ...
                if(ret){
                    WCN_DBG(FM_WARNING|D_MAIN, "MT6620_Rds_Tx_Enable failed!\n");
                    //FIX ME, how to handle this case?
                    goto out;
                }   
                ret = MT6620_Rds_Tx(cxt->txcxt.pi, cxt->txcxt.ps, cxt->txcxt.other_rds, cxt->txcxt.other_rds_cnt);
                if(ret){
                    WCN_DBG(FM_WARNING|D_MAIN, "MT6620_Rds_Tx failed!\n");
                    //FIX ME, how to handle this case?
                    goto out;
                }
                break;
            case FM_PWR_OFF:
                WCN_DBG(FM_WARNING|D_MAIN, "fm was pwr down, no need powerup!\n");
                break;
            default:
                WCN_DBG(FM_ERR|D_MAIN, "error para!\n");
                break;
        }
    }  
out:
    WCN_DBG(FM_NOTICE|D_MAIN,"-%s():[T=%d]\n", __func__, jiffies_to_msecs(jiffies));
    return ret;
}
...
static void fm_rst_recover_worker_func(struct work_struct *work)
{
    ...
    if(ret){
        WCN_DBG(FM_ERR|D_MAIN,"fm_recover_func failed\n");
    }
...
static void fm_subsys_recover_worker_func(struct work_struct *work)
{
    ...
    WCN_DBG(FM_NOTICE|D_MAIN,"+%s():\n", __func__);
    if(chip_rst_state != FM_WHOLECHIP_RST_OFF){
        WCN_DBG(FM_WARNING|D_MAIN,"FM whole chip resetting\n");
        goto out;
    }
    fm_cxt->ref = fm_cb->ref;
    fm_cxt->freq = _current_frequency;
        ...
        switch (state){
            case FM_TASK_RX_PARSER_PKT_TYPE:
                if(ch == FM_TASK_EVENT_PKT_TYPE){
                    if((i+5) < RX_BUF_SIZE){
                        WCN_DBG(FM_DEBUG|D_RX,"0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x \n", rx_buf[i], rx_buf[i+1], rx_buf[i+2], rx_buf[i+3], rx_buf[i+4], rx_buf[i+5]);
                    }else{
                        WCN_DBG(FM_DEBUG|D_RX,"0x%02x 0x%02x\n", rx_buf[i], rx_buf[i+1]);
                    }
                    state = FM_TASK_RX_PARSER_OPCODE;
                }else{
                    WCN_DBG(FM_ALERT|D_RX,"%s(): event pkt type error (rx_buf[%d] = 0x%02x)\n", __func__, i, ch);
                }
                i++;
                break;
            ...
            case FM_TASK_RX_PARSER_PKT_LEN_2:
                ...
                if(length > 0){
                    ...
                    WCN_DBG(FM_DEBUG|D_RX,"[flag=0x%08x]\n", flag);
                    wake_up(&fm_wq);
                }
                break;
                ...
                switch(opcode){
                    ...
                    case FM_SCAN_OPCODE:
                        ...
                        if((RX_BUF_SIZE - i) < (sizeof(uint16_t) * MT6620_SCANTBL_SIZE)){
                            WCN_DBG(FM_ALERT|D_RX,"FM_SCAN_OPCODE err, [tblsize=%d],[bufsize=%d]\n", (sizeof(uint16_t) * MT6620_SCANTBL_SIZE), (RX_BUF_SIZE - i));
                            wake_up(&fm_wq);
                            return;
                        }
                    ...
                    case RDS_RX_DATA_OPCODE:
                        flag |= (1 << opcode);
                        WCN_DBG(FM_INFO|D_RX,"-fm_rds_mutex\n");
                        if (down_interruptible(&fm_rds_mutex)){
                            WCN_DBG(FM_CRIT|D_RX,"Handle RDS: down rds mutex failed\n");
                            return;
                        }
                        rds_rx_size = length;
                        //check if the rds data is long enough
                        if((RX_BUF_SIZE - i) < rds_rx_size){
                            WCN_DBG(FM_ALERT|D_RX,"RDS_RX_DATA_OPCODE err, [rdsrxsize=%d],[bufsize=%d]\n", (int)rds_rx_size, (RX_BUF_SIZE - i));
                            goto rds_out;
                        }
                        memcpy(&rds_rx_result, &rx_buf[i], rds_rx_size);
                        WCN_DBG(FM_DEBUG|D_RX,"[rds_rx_size=%d]\n", rds_rx_size);
                        #if 0   //RDS Rx verification
                        {
                            ...
                            WCN_DBG(FM_NOTICE|D_RX,"%lu\t", jiffies * 1000 / HZ);
                            for(grp = 0; grp < 6; grp++, j+=12){
                                WCN_DBG(FM_NOTICE|D_RX,"%02x%02x%02x%02x%02x%02x%02x%02x\t%02x%02x%02x%02x\t", ptr[j], ptr[j+1], ptr[j+2], ptr[j+3], ptr[j+4], ptr[j+5], ptr[j+6], ptr[j+7], ptr[j+8], ptr[j+9], ptr[j+10], ptr[j+11]);
                            }
                            WCN_DBG(FM_NOTICE|D_RX,"\n");
                        }
                        #endif
...
rds_out:                        
                        ...
                        != RDS_EVENT_AF_LIST)){
                            WCN_DBG(FM_DEBUG|D_RX,"Notify user to read, [event:%04x]\n", pstRDSData->event_status);
                            fm->RDS_Data_ready = true;
                            wake_up_interruptible(&fm->read_wait);
                        }

                        WCN_DBG(FM_DEBUG|D_RX,"%s():[flag=0x%08x]\n", __func__, flag);
                        wake_up(&fm_wq);
                        break;
...
static int fm_setup_cdev(struct fm *fm)
{
    ...
    if (alloc_ret){
        WCN_DBG(FM_ALERT|D_ALL,"%s():fail to register chrdev\n", __func__);
        return alloc_ret;
    }
#if 0
    ...
    if (err) {
        WCN_DBG(FM_ALERT|D_MAIN,"alloc dev_t failed\n");
        return -1;
    }
#endif
    WCN_DBG(FM_NOTICE|D_MAIN,"alloc %s:%d:%d\n", FM_NAME, MAJOR(fm->dev_t), MINOR(fm->dev_t));
    ...
    if (err){
        WCN_DBG(FM_ALERT|D_ALL,"%s():alloc dev_t failed\n", __func__);
        return -1;
    }
                          ...
                          unsigned int cmd, unsigned long arg)
{
    ...
    WCN_DBG(FM_INFO|D_MAIN,"%s():\n", __func__);

    //check if we are resetting
    if(chip_rst_state != FM_WHOLECHIP_RST_OFF){
        WCN_DBG(FM_WARNING|D_MAIN, "whole chip is resetting, retry later\n");
    ...
    if(subsys_rst_state != FM_SUBSYS_RST_OFF){
        WCN_DBG(FM_WARNING|D_MAIN, "FM subsys is resetting, retry later\n");
        ret = -ERR_SUBSYS_RST;
        //ret = 0;
        return ret;
    }
    ...
    switch(cmd){
        case FM_IOCTL_POWERUP:{
            struct fm_tune_parm parm;
            WCN_DBG(FM_DEBUG|D_IOCTL,"......FM_IOCTL_POWERUP......\n");
// FIXME!!
//            if (!capable(CAP_SYS_ADMIN))
//                return -EPERM;
            if (copy_from_user(&parm, (void*)arg, sizeof(struct fm_tune_parm))){
                WCN_DBG(FM_ALERT|D_IOCTL|D_ALL,"copy_from_user error\n");
                return -EFAULT;
            }
            WCN_DBG(FM_INFO|D_IOCTL,"-fm_ops_mutex\n");
            if (down_interruptible(&fm_ops_mutex)){
                WCN_DBG(FM_ALERT|D_IOCTL|D_ALL,"get fm_ops_mutex error\n");
                return -EFAULT;
            }
            ret = fm_powerup(fm, &parm);
            if(-ERR_FW_NORES == ret){
                WCN_DBG(FM_ALERT|D_IOCTL|D_ALL,"fw no response, do hw reset\n");
                fm_hw_reset();
            }         
            up(&fm_ops_mutex);
            WCN_DBG(FM_INFO|D_IOCTL,"+fm_ops_mutex\n");
            if (copy_to_user((void*)arg, &parm, sizeof(struct fm_tune_parm))){
                WCN_DBG(FM_ALERT|D_IOCTL|D_ALL,"copy_to_user error\n");
                return -EFAULT;
            }
        ...
        case FM_IOCTL_POWERDOWN:{
            int32_t type = -1;
            WCN_DBG(FM_DEBUG|D_IOCTL,"......FM_IOCTL_POWERDOWN......\n");
            WCN_DBG(FM_DEBUG|D_IOCTL,"[chipon=%d],[powerup=%d],[ref=%d]\n", 
                fm->chipon, 
				fm->powerup, 
				fm->ref);
// FIXME!!
...
//                return -EPERM;
            if (FALSE == fm->chipon){
                WCN_DBG(FM_ALERT|D_IOCTL,"ERROR, FM chip already is off\n");
                return -EFAULT;
            }
        ...
        case FM_IOCTL_TUNE:{
            struct fm_tune_parm parm;
            WCN_DBG(FM_DEBUG|D_IOCTL,"......FM_IOCTL_TUNE......\n");
// FIXME!
//            if (!capable(CAP_SYS_ADMIN))
//                return -EPERM;
            if (FALSE == fm->chipon){
                WCN_DBG(FM_ALERT|D_IOCTL,"ERROR, FM chip is OFF\n");
                return -EFAULT;
            }
            if (copy_from_user(&parm, (void*)arg, sizeof(struct fm_tune_parm))){
                WCN_DBG(FM_ALERT|D_IOCTL|D_ALL,"copy_from_user error\n");
        ...
        case FM_IOCTL_SEEK:{
            ...
            WCN_DBG(FM_DEBUG|D_IOCTL,"......FM_IOCTL_SEEK......\n");
// FIXME!!
//            if (!capable(CAP_SYS_ADMIN))
//              return -EPERM;
            if (FALSE == fm->chipon){
        ...
        case FM_IOCTL_SCAN:{
            ...
            WCN_DBG(FM_DEBUG|D_IOCTL,"......FM_IOCTL_SCAN......\n");
        ...
        {
            uint32_t vol;
            WCN_DBG(FM_DEBUG|D_IOCTL,"......FM_IOCTL_SETVOL......\n");
// FIXME!!
//            if (!capable(CAP_SYS_ADMIN))
//              return -EPERM;
            if (FALSE == fm->chipon){
            ...
            WCN_DBG(FM_DEBUG|D_IOCTL,"FM_IOCTL_SETVOL, [vol=%d]\n", vol);
        ...
        case FM_IOCTL_GETVOL:{
            uint32_t vol;
            WCN_DBG(FM_DEBUG|D_IOCTL,"......FM_IOCTL_GETVOL......\n");
// FIXME!!
//            if (!capable(CAP_SYS_ADMIN))
//              return -EPERM;
		...
		case FM_IOCTL_IS_FM_POWERED_UP:{
			uint32_t pwredup = 0;
            WCN_DBG(FM_DEBUG|D_IOCTL,"......FM_IOCTL_IS_FM_POWERED_UP......\n");
            WCN_DBG(FM_INFO|D_IOCTL,"-fm_ops_mutex\n");
            if (down_interruptible(&fm_ops_mutex)){
                WCN_DBG(FM_ALERT|D_IOCTL|D_ALL,"get fm_ops_mutex error\n");
                return -EFAULT;
            ...
            up(&fm_ops_mutex);
            WCN_DBG(FM_INFO|D_IOCTL,"+fm_ops_mutex\n");
			WCN_DBG(FM_DEBUG|D_IOCTL,"FM_IOCTL_IS_FM_POWERED_UP,[powerup=%d]\n", pwredup);
        ...
        case FM_IOCTL_MUTE:{
            uint32_t bmute;
            WCN_DBG(FM_DEBUG|D_IOCTL,"......FM_IOCTL_MUTE......\n");
// FIXME!!
//            if (!capable(CAP_SYS_ADMIN))
//              return -EPERM;
            ...
            WCN_DBG(FM_DEBUG|D_IOCTL,"FM_IOCTL_MUTE:[mute=%d]\n", bmute);
            if (FALSE == fm->chipon){
                WCN_DBG(FM_ALERT|D_IOCTL,"ERROR, FM chip is OFF\n");
                return -EFAULT;
            }
        ...
        case FM_IOCTL_GETRSSI:{
            uint32_t rssi = 0;
            WCN_DBG(FM_DEBUG|D_IOCTL,"......FM_IOCTL_GETRSSI......\n");
// FIXME!!
//            if (!capable(CAP_SYS_ADMIN))
//              return -EPERM;
            if (FALSE == fm->chipon){
        ...
        case FM_IOCTL_RW_REG:{
            ...
            WCN_DBG(FM_DEBUG|D_IOCTL,"......FM_IOCTL_RW_REG......\n");
// FIXME!!
//            if (!capable(CAP_SYS_ADMIN))
//              return -EPERM;
            if (FALSE == fm->chipon){
        ...
        case FM_IOCTL_GETCHIPID:{
            uint16_t chipid;
            WCN_DBG(FM_DEBUG|D_IOCTL,"......FM_IOCTL_GETCHIPID......\n");
            ...
            chipid = fm->chip_id;
            WCN_DBG(FM_DEBUG|D_IOCTL,"FM_IOCTL_GETCHIPID:%04x\n", chipid);
        ...
        case FM_IOCTL_GETMONOSTERO:{
            ...
            WCN_DBG(FM_DEBUG|D_IOCTL,"......FM_IOCTL_GETMONOSTERO......\n");
            ...
            WCN_DBG(FM_DEBUG|D_IOCTL,"FM_IOCTL_GETMONOSTERO:%04x\n", usStereoMono);
            if(-ERR_FW_NORES == ret){
                WCN_DBG(FM_ALERT|D_IOCTL|D_ALL,"fw no response, do hw reset\n");
                fm_hw_reset();
            }
        ...
        case FM_IOCTL_GETCURPAMD:{
            uint16_t PamdLevl;
            WCN_DBG(FM_DEBUG|D_IOCTL,"......FM_IOCTL_GETCURPAMD......\n");
            ...
            WCN_DBG(FM_DEBUG|D_MAIN,"FM_IOCTL_GETCURPAMD:%d\n", PamdLevl);
            if(-ERR_FW_NORES == ret){
                WCN_DBG(FM_ALERT|D_IOCTL|D_ALL,"fw no response, do hw reset\n");
                fm_hw_reset();
            }
        ...
        case FM_IOCTL_EM_TEST:{
            ...
            WCN_DBG(FM_DEBUG|D_IOCTL,"......FM_IOCTL_EM_TEST......\n");
// FIXME!!
//            if (!capable(CAP_SYS_ADMIN))
//              return -EPERM;
            if (FALSE == fm->chipon){
		...
		case FM_IOCTL_RDS_SUPPORT:{
			int support;
            WCN_DBG(FM_DEBUG|D_IOCTL,"......FM_IOCTL_RDS_SUPPORT......\n");
        ...
        case FM_IOCTL_RDS_ONOFF:{
            ...
            WCN_DBG(FM_DEBUG|D_IOCTL,"......FM_IOCTL_RDS_ONOFF......\n");
            WCN_DBG(FM_DEBUG|D_IOCTL,"[chipon=%d] [powerup=%d] [ref=%d]\n", 
				fm->chipon, 
				fm->powerup, 
				fm->ref);
            ...
            if(rds_onoff){                
                if((ret = MT6620_RDS_OnOff(fm, true))){
                    WCN_DBG(FM_ALERT|D_IOCTL,"FM_IOCTL_RDS_ONOFF faield\n");
                    if(-ERR_FW_NORES == ret){
                        WCN_DBG(FM_ALERT|D_IOCTL|D_ALL,"fw no response, do hw reset\n");
                        fm_hw_reset();
                    }
                ...
                WCN_DBG(FM_DEBUG|D_MAIN,"FM_IOCTL_RDS_ON:%d\n", jiffies_to_msecs(jiffies));
            }else{
                //fm_disable_eint(); 
                fm->RDS_Data_ready = true;
                wake_up_interruptible(&fm->read_wait);
                ...
                WCN_DBG(FM_DEBUG|D_MAIN,"FM_IOCTL_RDS_OFF:%d\n", jiffies_to_msecs(jiffies));        
            }
            if(-ERR_FW_NORES == ret){
                WCN_DBG(FM_ALERT|D_IOCTL|D_ALL,"fw no response, do hw reset\n");
                fm_hw_reset();
	    ...
	    case FM_IOCTL_GETGOODBCNT:{
		    uint16_t uGBLCNT = 0;	
            WCN_DBG(FM_DEBUG|D_IOCTL,"......FM_IOCTL_GETGOODBCNT......\n");
            ...
            WCN_DBG(FM_INFO|D_IOCTL,"-fm_read_mutex\n");
	        if (down_interruptible(&fm_read_mutex)){
                WCN_DBG(FM_ALERT|D_IOCTL|D_ALL,"get fm_ops_mutex error\n");
                return -EFAULT;
            }
		    ...
		    up(&fm_read_mutex);
            WCN_DBG(FM_INFO|D_IOCTL,"+fm_read_mutex\n");
            WCN_DBG(FM_DEBUG|D_IOCTL,"FM_IOCTL_GETGOODBCNT:%d\n", uGBLCNT);
        ...
        case FM_IOCTL_GETBADBNT:{
            ...
            WCN_DBG(FM_DEBUG|D_IOCTL,"......FM_IOCTL_GETBADBNT......\n");
			...
			WCN_DBG(FM_DEBUG|D_IOCTL,"FM_IOCTL_GETBADBNT:%d\n", uBadBLCNT);
        ...
        case FM_IOCTL_GETBLERRATIO:{
            ...
            WCN_DBG(FM_DEBUG|D_IOCTL,"......FM_IOCTL_GETBLERRATIO......\n");
			...
			WCN_DBG(FM_DEBUG|D_IOCTL,"FM_IOCTL_GETBLERRATIO:%d\n", uBlerRatio);
        ...
        case FM_IOCTL_DUMP_REG:{
            WCN_DBG(FM_DEBUG|D_IOCTL,"......FM_IOCTL_DUMP_REG......\n");
        ...
        case FM_IOCTL_POWERUP_TX:{
            struct fm_tune_parm parm;
            WCN_DBG(FM_DEBUG|D_IOCTL,"......FM_IOCTL_POWERUP_TX......\n");
// FIXME!!
//            if (!capable(CAP_SYS_ADMIN))
//                return -EPERM;
            if (copy_from_user(&parm, (void*)arg, sizeof(struct fm_tune_parm))){
        ...
        case FM_IOCTL_TUNE_TX:{
            struct fm_tune_parm parm;
            WCN_DBG(FM_DEBUG|D_IOCTL,"......FM_IOCTL_TUNE_TX......\n");
// FIXME!
//            if (!capable(CAP_SYS_ADMIN))
//                return -EPERM;
            if (FALSE == fm->chipon){
        ...
        case FM_IOCTL_TX_SUPPORT:{
            ...
            WCN_DBG(FM_DEBUG|D_IOCTL,"......FM_IOCTL_TX_SUPPORT......\n");
// FIXME!!
//            if (!capable(CAP_SYS_ADMIN))
//                return -EPERM;
            WCN_DBG(FM_INFO|D_IOCTL,"-fm_ops_mutex\n");
			...
			if(FM_TX_SUPPORT == atomic_read(&fm->tx_support)){
		   		...
		   		tx_support = -1;
				WCN_DBG(FM_ALERT|D_IOCTL,"FM_IOCTL_TX_SUPPORT:invalid flag[fm->tx_support=%d]\n", 
		...
		case FM_IOCTL_RDSTX_ENABLE:{
            ...
            WCN_DBG(FM_DEBUG|D_IOCTL,"......FM_IOCTL_RDSTX_ENABLE......\n");
// FIXME!!
//            if (!capable(CAP_SYS_ADMIN))
//                return -EPERM;
			if (FALSE == fm->chipon){
			...
			if(FM_RDS_TX_ENABLE == rds_tx_enable){
				...
				WCN_DBG(FM_ALERT|D_IOCTL,"FM_IOCTL_RDSTX_ENABLE:invalid para[rds_tx_enable=%d]\n", 
					rds_tx_enable);
				ret = -1;
		    }		
            up(&fm_ops_mutex);
		...
		case FM_IOCTL_RDSTX_SUPPORT:{
            ...
            WCN_DBG(FM_DEBUG|D_IOCTL,"......FM_IOCTL_RDSTX_SUPPORT......\n");
// FIXME!!
//            if (!capable(CAP_SYS_ADMIN))
//                return -EPERM;
            WCN_DBG(FM_INFO|D_IOCTL,"-fm_ops_mutex\n");
			...
			if(FM_RDS_TX_SUPPORT == atomic_read(&fm->rds_tx_support)){
		   		...
		   		rds_tx_support = -1;
				WCN_DBG(FM_ALERT|D_IOCTL,"FM_IOCTL_RDSTX_SUPPORT:invalid flag[fm->rds_tx_support=%d]\n", 
		...
		case FM_IOCTL_TX_SCAN:{
            ...
            WCN_DBG(FM_DEBUG|D_IOCTL,"......FM_IOCTL_TX_SCAN......\n");
// FIXME!!
//            if (!capable(CAP_SYS_ADMIN))
//                return -EPERM;
            if (FALSE == fm->chipon){
			...
			if(ret < 0){
				WCN_DBG(FM_ALERT|D_MAIN,"FM_IOCTL_TX_SCAN failed\n");
			}
            if(-ERR_FW_NORES == ret){
                WCN_DBG(FM_ALERT|D_IOCTL|D_ALL,"fw no response, do hw reset\n");
                fm_hw_reset();
        ...
        case FM_IOCTL_RDS_TX:{
            ...
            WCN_DBG(FM_DEBUG|D_IOCTL,"......FM_IOCTL_RDS_TX......\n");
// FIXME!!
//            if (!capable(CAP_SYS_ADMIN))
//                return -EPERM;
            if (FALSE == fm->chipon){
        ...
        case FM_IOCTL_OVER_BT_ENABLE:{
			int32_t fm_via_bt = -1;
            WCN_DBG(FM_DEBUG|D_IOCTL,"......FM_IOCTL_OVER_BT_ENABLE......\n");
			...
			if(FM_OVER_BT_ENABLE == fm_via_bt){
				...
				WCN_DBG(FM_ALERT|D_IOCTL,"FM_IOCTL_OVER_BT_ENABLE:invalid para[fm_over_bt=%d]\n", 
					fm_via_bt);
				ret = -1;
		    }		
            up(&fm_ops_mutex);
        ...
        case FM_IOCTL_ANA_SWITCH:{
            int32_t antenna = -1;
            WCN_DBG(FM_DEBUG|D_IOCTL,"......FM_IOCTL_ANA_SWITCH......\n");
        ...
        case FM_IOCTL_GETCAPARRAY:{
            int caparray;
            WCN_DBG(FM_DEBUG|D_IOCTL,"......FM_IOCTL_GETCAPARRAY......\n");
            ...
            WCN_DBG(FM_DEBUG|D_IOCTL,"FM_IOCTL_GETMONOSTERO:%x\n", caparray);
            if(-ERR_FW_NORES == ret){
                WCN_DBG(FM_ALERT|D_IOCTL|D_ALL,"fw no response, do hw reset\n");
                fm_hw_reset();
            }
        ...
        case FM_IOCTL_GPS_RTC_DRIFT:{
            ...
            WCN_DBG(FM_DEBUG|D_IOCTL,"......FM_IOCTL_GPS_RTC_DRIFT......\n");
            ...
            if(ret){
                WCN_DBG(FM_ALERT|D_IOCTL|D_ALL,"fm_get_gps_rtc_info error\n");
                return ret;
            }
            break;
        }
        ...
        case FM_IOCTL_I2S_SETTING:{
            ...
            WCN_DBG(FM_DEBUG|D_IOCTL,"......FM_IOCTL_I2S_SETTING......\n");
            ...
            if(ret){
                WCN_DBG(FM_ALERT|D_IOCTL|D_ALL,"MT6620_I2S_Setting error\n");
                return ret;
            }
            break;
        }
        case FM_IOCTL_RDS_GROUPCNT:{
            ...
            WCN_DBG(FM_DEBUG|D_IOCTL,"......FM_IOCTL_RDS_GROUPCNT......\n");
        ...
        case FM_IOCTL_RDS_GET_LOG:{
            ...
            WCN_DBG(FM_DEBUG|D_IOCTL,"......FM_IOCTL_RDS_GET_LOG......\n");
            rds_log.dirty = TRUE;
        ...
        default:
            WCN_DBG(FM_ALERT|D_IOCTL,"Invalid IOCTL[cmd=%d], please check!\n", cmd);
            return -EPERM;
    } 
    return ret;
}
...
static ssize_t fm_ops_read(struct file *filp, char *buf, size_t len, loff_t *off)
{
    ...
    WCN_DBG(FM_DEBUG|D_MAIN,"+%s():\n", __func__);
    if (!fm){
        WCN_DBG(FM_ALERT|D_MAIN,"%s():invalid fm pointer\n", __func__);
        return 0;
    }
    
    if(!buf || len < sizeof(RDSData_Struct)){
        WCN_DBG(FM_ERR|D_ALL,"%s():invalid buf\n", __func__);
        return 0;
    }    
...
RESTART:    
    if((fm->RDS_Data_ready == true) || (fm->powerup == FM_PWR_OFF)){
        ...
        WCN_DBG(FM_INFO|D_MAIN,"-fm_rds_mutex\n");
        if (down_interruptible(&fm_rds_mutex)){
            WCN_DBG(FM_ALERT|D_MAIN,"get fm_rds_mutex error\n");
            return 0;       
        }
        ...
        if((left = copy_to_user((void *)buf, fm->pstRDSData, (unsigned long)copy_len))){
            WCN_DBG(FM_ALERT|D_MAIN,"%s():copy_to_user failed\n", __func__);
        }
        WCN_DBG(FM_DEBUG|D_MAIN,"copy len=%d\n", (copy_len-left));
         //Clear
        if(left == 0){
        ...
        WCN_DBG(FM_DEBUG|D_MAIN,"wait event\n");
        if (wait_event_interruptible(fm->read_wait, (fm->RDS_Data_ready == true)) == 0){
            WCN_DBG(FM_DEBUG|D_MAIN,"wait event success[%d]\n", fm->RDS_Data_ready);
            goto RESTART;             
        }else {
            WCN_DBG(FM_ALERT|D_MAIN,"%s():wait event err[%d]\n", __func__, fm->RDS_Data_ready);
            fm->RDS_Data_ready = false;
            return 0;            
        }
    }
...
static int fm_ops_open(struct inode *inode, struct file *filp)
{
    ...
    WCN_DBG(FM_DEBUG|D_MAIN,"+%s():[T=%d]\n", __func__, jiffies_to_msecs(jiffies));

    //check if we are resetting
    if(chip_rst_state != FM_WHOLECHIP_RST_OFF){
        WCN_DBG(FM_WARNING|D_MAIN, "FM is resetting, please open later\n");
        ret = -ERR_WHOLECHIP_RST;
        return ret;
    }
    ...
    WCN_DBG(FM_INFO|D_MAIN,"-fm_ops_mutex\n");
    if (down_interruptible(&fm_ops_mutex)){
        WCN_DBG(FM_ALERT|D_IOCTL|D_ALL,"get fm_ops_mutex error\n");
        return -EFAULT;
    }
    ...
    fm->ref++;
    WCN_DBG(FM_NOTICE|D_MAIN,"%s [fm->ref=%d]\n", __func__, fm->ref);
    //Audio driver will open first handle, for save power.
    if(fm->ref > 0){
        if (FALSE == fm->chipon){
            //Turn on FM on 6620 chip by WMT driver 
            if (MTK_WCN_BOOL_FALSE == mtk_wcn_wmt_func_on(WMTDRV_TYPE_FM)) {
                WCN_DBG(FM_ALERT|D_MAIN,"%s WMT turn on FM fail!\n", __func__);
                ret = -ENODEV;
                goto out; 
            }
            fm->chipon = true;
            fm_cxt->chipon = true;
            WCN_DBG(FM_NOTICE|D_MAIN,"%s WMT turn on FM OK, [fm->chipon=%d]!\n", __func__, fm->chipon);
        }
    }
    
    /* GeorgeKuo: turn on function before check stp ready */
    if( FALSE == mtk_wcn_stp_is_ready()){
        WCN_DBG(FM_ALERT|D_MAIN,"6620 stp is not ready, please retry later\n");
        ret = -ENODEV;
        goto out; 
    }
...
static int fm_ops_release(struct inode *inode, struct file *filp)
{
    ...
    WCN_DBG(FM_INFO|D_MAIN,"-fm_ops_mutex\n");
    if (down_interruptible(&fm_ops_mutex)){
        WCN_DBG(FM_ALERT|D_MAIN,"get fm_ops_mutex error\n");
        return -EFAULT;
    }
    ...
    if(fm->ref > 0){
        ...
        WCN_DBG(FM_ALERT|D_MAIN,"%s():open and close FM unmatch\n", __func__);
        ret = -EFAULT;
    }
    WCN_DBG(FM_NOTICE|D_MAIN,"%s [fm->ref=%d]\n", __func__, fm->ref);
    ...
    //check if we are resetting
    if(chip_rst_state != FM_WHOLECHIP_RST_OFF){
        WCN_DBG(FM_WARNING|D_MAIN, "FM is resetting, only ref--\n");
        ret = -ERR_WHOLECHIP_RST;
        up(&fm_ops_mutex);
        WCN_DBG(FM_INFO|D_MAIN,"+fm_ops_mutex\n"); 
        return ret;
    ...
    if(subsys_rst_state != FM_SUBSYS_RST_OFF){
        WCN_DBG(FM_WARNING|D_MAIN, "FM subsys is resetting, only ref--\n");
        ret = -ERR_SUBSYS_RST;
        up(&fm_ops_mutex);
        WCN_DBG(FM_INFO|D_MAIN,"+fm_ops_mutex\n");
        return ret;
...
static int fm_get_process_path(struct fm *fm)
{
    ...
    WCN_DBG(FM_NOTICE|D_MAIN,"[TaskName=%s],[TaskId=%d]\n", current->comm, current->pid);
...
static int fm_print_cfg_info(struct fm *fm, void* data)
{
    ...
    WCN_DBG(FM_NOTICE|D_INIT,"******fm config info******\n");
    WCN_DBG(FM_NOTICE|D_INIT,"***chip:\tMT6620\t\n");
    WCN_DBG(FM_NOTICE|D_INIT,"***band:\t%d\t\n", FMR_BAND);
    WCN_DBG(FM_NOTICE|D_INIT,"***freq_min:\t%d\t\n", FMR_BAND_FREQ_L);
    WCN_DBG(FM_NOTICE|D_INIT,"***freq_max:\t%d\t\n", FMR_BAND_FREQ_H);
    WCN_DBG(FM_NOTICE|D_INIT,"***scan_tbl:\t%d\t\n", FMR_SCAN_CH_SIZE);
    WCN_DBG(FM_NOTICE|D_INIT,"***space:\t%d\t\n", FMR_SEEK_SPACE);
    WCN_DBG(FM_NOTICE|D_INIT,"***rssi_long:\t0x%04x\t\n", FMR_RSSI_TH_LONG);
    WCN_DBG(FM_NOTICE|D_INIT,"***rssi_short:\t0x%04x\t\n", FMR_RSSI_TH_SHORT);
    WCN_DBG(FM_NOTICE|D_INIT,"***CQI:\t0x%04x\t\n", FMR_CQI_TH);
    WCN_DBG(FM_NOTICE|D_INIT,"******fm config end******\n");
...
static int fm_init()
{
    ...
    if (!(fm = kzalloc(sizeof(struct fm), GFP_KERNEL))){
        WCN_DBG(FM_ALERT|D_INIT,"kzalloc(fm) -ENOMEM\n");
        err = -ENOMEM;
        goto fm_alloc_err;
    }

    if (!(tx_buf = kzalloc(TX_BUF_SIZE + 1, GFP_KERNEL))){
        WCN_DBG(FM_ALERT|D_INIT,"kzalloc(tx_buf) -ENOMEM\n");
        err = -ENOMEM;
        goto tx_alloc_err;
    }

    if (!(fm_cxt = kzalloc(sizeof(struct fm_context), GFP_KERNEL))){
        WCN_DBG(FM_ALERT|D_INIT,"kzalloc(fm_cxt) -ENOMEM\n");
        err = -ENOMEM;
        goto tx_alloc_err;
    }
    ...
    if(!fm->fm_workqueue){
        WCN_DBG(FM_ALERT|D_INIT,"-ENOMEM for fm_workqueue\n");
        err = -ENOMEM;
        goto rx_wq_err;
    }
    ...
    if(!fm->fm_timer_workqueue){
        WCN_DBG(FM_ALERT|D_INIT,"-ENOMEM for fm_timer_workqueue\n");
        err = -ENOMEM;
    ...
    if(err){
        WCN_DBG(FM_ALERT|D_INIT,"print fm cfg info failed\n");    
    }
    return 0;
...
static int do_cmd_send(struct fm *fm, unsigned char *buf, int len, int opcode, void* data){
    ...
    if (down_interruptible(&fm_cmd_mutex)){
        WCN_DBG(FM_ALERT|D_MAIN,"%s(): get mutex failed\n", __func__);
        ret = -ERR_GET_MUTEX;
        return ret;
    }
    ...
    if(buf && (len > 4) && (len < TX_BUF_SIZE)){
        WCN_DBG(FM_NOTICE|D_MAIN,"sending special cmd[len=%d]\n", len);
        ret = fm_send_wait_timeout(buf, (uint16_t)len, 
                                    FLAG_EN, SW_RETRY_CNT, EN_TIMEOUT);
        if(ret){
            WCN_DBG(FM_ALERT|D_MAIN,"send special cmd failed\n");
            goto out;
        }
    }
    ...
    if(tx_buf == NULL){
        WCN_DBG(FM_ALERT|D_MAIN,"%s(): invalid tx_buf\n", __func__);
        ret = -ERR_INVALID_BUF;
		goto out;
	}
	memset(tx_buf, 0, TX_BUF_SIZE);
    ...
    if(pkt_size < 0){
       WCN_DBG(FM_ALERT|D_MAIN,"unsupported cmd\n"); 
       goto out;
    }
    WCN_DBG(FM_NOTICE|D_MAIN,"sending additional cmd[len=%d]\n", pkt_size);
    ret = fm_send_wait_timeout(tx_buf, (uint16_t)pkt_size, 
                                    FLAG_EN, SW_RETRY_CNT, EN_TIMEOUT);
    if(ret){
        WCN_DBG(FM_ALERT|D_MAIN,"send additional cmd failed\n");
    }
...
static int fm_powerup(struct fm *fm, struct fm_tune_parm *parm)
{
    ...
    if(FM_PWR_RX_ON == fm->powerup){
        //FM RX already on
        WCN_DBG(FM_NOTICE|D_MAIN,"%s, FM Rx already powered up\n", __func__);
        ...
        ret = fm_powerdown(fm, FM_TX);
        if(ret){
           WCN_DBG(FM_ALERT|D_MAIN,"FM pwr down Tx fail!\n"); 
           return ret;
        }
    }
    ...
    if (FALSE == fm->chipon){
        if (MTK_WCN_BOOL_FALSE == mtk_wcn_wmt_func_on(WMTDRV_TYPE_FM)){
            WCN_DBG(FM_ALERT|D_MAIN,"WMT turn on FM fail!\n");
            return -ENODEV;
        }
        WCN_DBG(FM_NOTICE|D_MAIN,"WMT turn on FM OK!\n");
        fm->chipon = TRUE;
    }

    WCN_DBG(FM_INFO|D_MAIN,"+MT6620 power on procedure\n");
    if (down_interruptible(&fm_cmd_mutex)){
        WCN_DBG(FM_ALERT|D_MAIN,"%s(): get mutex failed\n", __func__);
        ret = -ERR_GET_MUTEX;
        return ret;
    ...
    ret = fm_send_wait_timeout(tx_buf, pkt_size, 
                                    FLAG_EN, SW_RETRY_CNT, EN_TIMEOUT);
    if(ret){
        WCN_DBG(FM_ALERT|D_MAIN,"%s(): mt6620_off_2_longANA failed\n", __func__);
        goto release_mutex;
    }
    ...
    ret = fm_send_wait_timeout(tx_buf, pkt_size, 
                                    FLAG_EN, SW_RETRY_CNT, EN_TIMEOUT);
    if(ret){
        WCN_DBG(FM_ALERT|D_MAIN,"%s(): mt6620_rx_digital_init failed\n", __func__);
        goto release_mutex;
    }
...
release_mutex:
  ...
  #endif
    if(ret){
        WCN_DBG(FM_ALERT|D_MAIN,"pwron set I2S on error\n");
        return ret;
    }
    WCN_DBG(FM_DEBUG|D_MAIN,"pwron set I2S on ok\n");
#endif

	WCN_DBG(FM_INFO|D_MAIN,"-MT6620 power on procedure\n");
    WCN_DBG(FM_NOTICE|D_MAIN,"pwron RX ok\n");
    ...
    if ((ret = fm_tune(fm, parm))){
        WCN_DBG(FM_ALERT|D_MAIN,"Power on RX tune failed\n");
        return ret;
    }
    parm->err = FM_SUCCESS;  
...
static int fm_antenna_switch(struct fm *fm, int antenna)
{
	...
	if(ret){
		WCN_DBG(FM_ALERT|D_MAIN,"Switch antenna failed\n");
	}
...
static int fm_powerup_tx(struct fm *fm, struct fm_tune_parm *parm)
{
    ...
    if(FM_PWR_TX_ON == fm->powerup){
        //FM TX already on
        WCN_DBG(FM_NOTICE|D_MAIN,"%s, FM Tx already powered up\n", __func__);
        parm->err = FM_BADSTATUS;
        return 0;
        ...
        ret = fm_powerdown(fm, FM_RX);
        if(ret){
           WCN_DBG(FM_ALERT|D_MAIN,"FM pwr down Rx fail!\n"); 
           return ret;
        }
    }
    //for normal case
    if (FALSE == fm->chipon){
        ...
        WCN_DBG(FM_DEBUG|D_MAIN,"WMT turn on FM OK!\n");
        fm->chipon = TRUE;
    }

    WCN_DBG(FM_INFO|D_MAIN,"+MT6620 power on tx procedure\n");
    if (down_interruptible(&fm_cmd_mutex)){
        WCN_DBG(FM_ALERT|D_MAIN,"%s(): get mutex failed\n", __func__);
        ret = -ERR_GET_MUTEX;
        return ret;
    ...
    ret = fm_send_wait_timeout(tx_buf, pkt_size, 
                                    FLAG_EN, SW_RETRY_CNT, EN_TIMEOUT);
    if(ret){
        WCN_DBG(FM_ALERT|D_MAIN,"%s(): mt6620_off_2_tx_shortANA failed\n", __func__);
        goto release_mutex;
    }
    ...
    ret = fm_send_wait_timeout(tx_buf, pkt_size, 
                                    FLAG_EN, SW_RETRY_CNT, EN_TIMEOUT);
    if(ret){
        WCN_DBG(FM_ALERT|D_MAIN,"%s(): mt6620_dig_init failed\n", __func__);
        goto release_mutex;
    }
release_mutex:
    up(&fm_cmd_mutex);
	...
	WCN_DBG(FM_INFO|D_MAIN,"-MT6620 power on tx procedure\n");
    WCN_DBG(FM_NOTICE|D_MAIN,"pwron tx ok\n");
    ...
    if(mtk_wcn_wmt_therm_ctrl(WMTTHERM_ENABLE) != TRUE){
       WCN_DBG(FM_ERR|D_MAIN,"wmt_therm_ctrl, WMTTHERM_ENABLE failed\n"); 
       ret = -ERR_STP;
       return ret;
    }
    if ((ret = fm_tune_tx(fm, parm))){
        WCN_DBG(FM_ALERT|D_MAIN,"PowerUp Tx(), tune failed\n");
        return ret;
    }
  ...
  #endif
    if(ret){
        WCN_DBG(FM_ALERT|D_MAIN,"pwron Tx set I2S on error\n");
        return ret;
    }
    WCN_DBG(FM_DEBUG|D_MAIN,"pwron Tx set I2S on ok\n");
#endif
...
static int fm_powerdown(struct fm *fm, int type)
{
    ...
    if(((fm->powerup == FM_PWR_TX_ON) && (type == FM_RX)) || ((fm->powerup == FM_PWR_RX_ON) && (type == FM_TX))){
        WCN_DBG(FM_NOTICE|D_MAIN,"no need do pwr down\n");
        ret = 0;
        goto out;
    }
    ...
    if(MT6620_I2S_Setting(FM_I2S_OFF, FM_I2S_MASTER, FM_I2S_48K)){
        WCN_DBG(FM_ALERT|D_MAIN,"pwrdown set I2S off error\n");
        goto out;
    }
    WCN_DBG(FM_DEBUG|D_MAIN,"pwrdown set I2S off ok\n");
#endif
	...
	if(tx_buf == NULL){
		WCN_DBG(FM_ALERT|D_MAIN,"invalid tx_buf\n");
        up(&fm_cmd_mutex);
		goto out;
	}
	memset(tx_buf, 0, TX_BUF_SIZE);
    ...
    WCN_DBG(FM_INFO|D_CMD,"+fm_cmd_mutex\n");
    if(ret){
        WCN_DBG(FM_ALERT|D_MAIN,"%s(): pwrdown failed\n", __func__);
        goto out;
    }
    ...
    if(fm->powerup == FM_PWR_TX_ON){
        if(mtk_wcn_wmt_therm_ctrl(WMTTHERM_DISABLE) != TRUE){
            WCN_DBG(FM_ERR|D_MAIN,"wmt_therm_ctrl, WMTTHERM_DISABLE failed\n"); 
            ret = -ERR_STP;
        }
    }
    ...
    WCN_DBG(FM_NOTICE|D_MAIN,"pwrdown %s ok\n", (type==FM_TX)? "TX" : "RX");
out:
    up(&fm_rxtx_mutex);
    return ret;
}
...
static int fm_seek(struct fm *fm, struct fm_seek_parm *parm)
{
    ...
    if (parm->band == FM_BAND_UE){
        ...
        WCN_DBG(FM_ALERT|D_MAIN,"band:%d out of range\n", parm->band);
        parm->err = FM_EPARM;
        return -EPERM;
    }

    if (parm->freq < fm->min_freq || parm->freq > fm->max_freq){
        WCN_DBG(FM_ALERT|D_MAIN,"freq:%d out of range\n", parm->freq);
        parm->err = FM_EPARM;
        return -EPERM;
    }
...
#ifdef FMDEBUG
    if (parm->seekdir == FM_SEEK_UP)
        WCN_DBG(FM_DEBUG|D_MAIN,"seek %d up\n", parm->freq);
    else
        WCN_DBG(FM_DEBUG|D_MAIN,"seek %d down\n", parm->freq);
#endif
...
static int  fm_scan(struct fm *fm, struct fm_scan_parm *parm)
{
    ...
    if(parm->band == FM_BAND_UE){
        ...
        WCN_DBG(FM_ERR|D_ALL,"fm_scan band:%d out of range\n", parm->band);
        parm->err = FM_EPARM;
        return -EPERM;
    }
    ...
    if(!(ret = MT6620_Scan(fm->min_freq, fm->max_freq, &(parm->freq), 
        ...
        WCN_DBG(FM_ALERT|D_MAIN,"fm_scan failed\n");
        parm->err = FM_SEEK_FAILED;
    }
...
static int fm_print_curCQI(uint16_t cur_freq)
{
    ...
    WCN_DBG(FM_NOTICE|D_MAIN,"FREQ=%d, RSSI=0x%04x, PAMD=0x%04x, MR=0x%04x\n", (int)cur_freq, rssi, pamd, mr);
    return ret;    
}
...
static int fm_tx_scan(struct fm *fm, struct fm_tx_scan_parm *parm)
{
    ...
    if (!fm->powerup){
        parm->err = FM_BADSTATUS;
		WCN_DBG(FM_ERR|D_ALL,"fm_tx_scan failed, [fm->powerup=%d]\n", fm->powerup);
        return -EPERM;
    }
	
	if(FM_TX_SUPPORT != atomic_read(&fm->tx_support)){
		parm->err = FM_BADSTATUS;
		WCN_DBG(FM_ERR|D_ALL,"fm_tx_scan failed, [fm->tx_support=%d]\n", atomic_read(&fm->tx_support));
		return -EPERM;
	}
	...
	switch(parm->band){
		...
		default:
			parm->err = FM_EPARM;
			WCN_DBG(FM_ERR|D_ALL,"fm_tx_scan: bad band para\n");
        	return -EPERM;
			break;
	}
    ...
    if(ret){
        WCN_DBG(FM_ALERT|D_MAIN,"switch to short ana failed\n");
        return ret;
    }
	...
	if(!(ret = MT6620_TxScan(fm->min_freq, fm->max_freq, &(parm->freq), 
        ...
        WCN_DBG(FM_ALERT|D_MAIN,"fm_tx_scan failed\n");
...
{
    int ret = 0;
    WCN_DBG(FM_DEBUG|D_RDS,"+%s()\n", __func__);
    if(parm->other_rds_cnt > 29){
        parm->err = FM_EPARM;
        WCN_DBG(FM_WARNING|D_RDS,"%s(), [other_rds_cnt=%d]\n", __func__, parm->other_rds_cnt);
        ret = -ERR_INVALID_PARA;
        goto out;
    }
    ...
    ret = MT6620_Rds_Tx_Enable(1);
    if(ret){
        WCN_DBG(FM_WARNING|D_RDS,"%s(), MT6620_Rds_Tx_Enable failed!\n", __func__);
        goto out;
    }   
...
static int fm_over_bt(struct fm *fm, int enable)
{
    ...
    if(ret){
        WCN_DBG(FM_WARNING|D_MAIN,"%s(),failed!\n", __func__);
    }
...
static int fm_proc_read(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	...
	WCN_DBG(FM_DEBUG|D_MAIN, "Enter fm_proc_read.\n");
	if(off != 0)
		return 0;
	...
	*eof = 1;
	WCN_DBG(FM_DEBUG|D_MAIN, "Leave fm_proc_read. FM_on = %s\n", page);
	return cnt;
}
...
static int fm_proc_write(struct file *file, const char *buffer, unsigned long count, void *data)
{
	...
	if (copy_from_user(tmp_buf, buffer, copysize)){
		WCN_DBG(FM_ERR|D_ALL, "failed copy_from_user\n");
        return -EFAULT;
    }

	if (sscanf(tmp_buf, "%x", &g_dbg_level) != 1){
		WCN_DBG(FM_ERR|D_ALL, "failed g_dbg_level = 0x%x\n", g_dbg_level);
		return -EFAULT;
	}

	WCN_DBG(FM_WARNING|D_MAIN, "success g_dbg_level = 0x%x\n", g_dbg_level);
	return count;
}
...
static int fmconfig_proc_read(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	...
	WCN_DBG(FM_DEBUG|D_MAIN, "Enter fm_config_proc_read.\n");
	if(off != 0)
		return 0;
	...
	*eof = 1;
	WCN_DBG(FM_DEBUG|D_MAIN, "Leave fm_config_proc_read. Config = %s\n", page);
	return cnt;
}
            ...
            switch(tmp_buf[1]){
                case 'u':
                    ...
                    if (sscanf(&tmp_buf[2], "%d", &g_fm_config.vcoon) != 1){
		                WCN_DBG(FM_ERR|D_ALL, "failed vcoon = %d\n", g_fm_config.vcoon);
		                return -EFAULT;
	                }
                ...
                case 'd':
	                ...
	                WCN_DBG(FM_WARNING|D_MAIN, "success vcooff = %d\n", g_fm_config.vcooff);
                    break;
                default:
                    WCN_DBG(FM_ERR|D_MAIN, "Para 2 error\n");
                    break;
            }
            break;
        case 't':
            if (sscanf(&tmp_buf[1], "%d", &g_fm_config.timer) != 1){
		        WCN_DBG(FM_ERR|D_ALL, "failed timer = %d\n", g_fm_config.timer);
		        return -EFAULT;
	        }
	        ...
	        WCN_DBG(FM_WARNING|D_MAIN, "success timer = %d\n", g_fm_config.timer);
            break;
        case 'r':
            if (sscanf(&tmp_buf[1], "%d", &g_fm_config.rdsrst) != 1){
		        WCN_DBG(FM_ERR|D_ALL, "failed rdsrst = %d\n", g_fm_config.rdsrst);
		        return -EFAULT;
	        }
	        ...
	        WCN_DBG(FM_WARNING|D_MAIN, "success rdsrst = %d\n", g_fm_config.rdsrst);
            break;
        case 'p':
            if (sscanf(&tmp_buf[1], "%d", &g_fm_config.txpwrctl) != 1){
		        WCN_DBG(FM_ERR|D_ALL, "failed txpwrctl = %d\n", g_fm_config.txpwrctl);
		        return -EFAULT;
	        }
	        ...
	        WCN_DBG(FM_WARNING|D_MAIN, "success txpwrctl = %d\n", g_fm_config.txpwrctl);
            break;
        case 's':
            fm_hw_reset();
	        WCN_DBG(FM_WARNING|D_MAIN, "do fm_hw_reset\n");
            break;
        case 'm':
            do_cmd_send(NULL, NULL, 0, FM_COM_CMD_TEST, NULL);
            break;
        default:
            WCN_DBG(FM_ERR|D_MAIN, "Para 1 error\n");
            break;
    }
	return count;
}
...
int fm_priv_register(struct fm_priv *pri, struct fm_op *op)
{
    ...
    WCN_DBG(FM_NOTICE|D_INIT,"%s(), [pri=0x%08x][op=0x%08x]\n", __func__, (unsigned int)pri, (unsigned int)op);
	FMR_ASSERT(pri);
	FMR_ASSERT(op);
...
static int __init mt_fm_probe(void)
{
    ...
    WCN_DBG(FM_NOTICE|D_INIT,"%s()\n", __func__);
    if ((err = fm_init())){
        WCN_DBG(FM_EMERG|D_ALL, "fm_init ERR:%d\n", err);
    }
