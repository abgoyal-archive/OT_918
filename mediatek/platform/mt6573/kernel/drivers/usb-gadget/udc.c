...
/*enable clock automatically while accessing register if the clock is disable, 
  the option should be off in MT6573E2*/
...
/* flushes contents of a fifo, needs to obtain spinlock of udc for exported 
version */
static void mt_ep_fifo_flush(struct usb_ep *_ep)
{
    if(!_ep){
