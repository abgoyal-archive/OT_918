...
static int hci_stp_open(struct hci_dev *hdev)
{
    ...
    */
    ...
    //unsigned char bt_bd_addr[10]={0x01,0x1a,0xfc,0x06,0x00,0x55,0x66,0x77,0x88,0x00};
    ...
    if (mtk_wcn_stp_is_ready()) {
        ...
        if(!mtk_wcn_stp_is_sdio_mode()){
            ...
            BT_INFO_FUNC("Reset bd_addr =>\n");        
            mtk_wcn_stp_send_data(g_bt_bd_addr,10,0);		
            msleep(100);		
            BT_INFO_FUNC("Reset bd_addr <=\n");	
        }

        BT_INFO_FUNC("Send pcm command to configure =>\n");			
        mtk_wcn_stp_send_data(bt_pcm,5,BT_TASK_INDX);/*WMT*/		
        msleep(100);		
        BT_INFO_FUNC("Send pcm command to configure <=\n");
...
static int __init hci_stp_init(void)
{
	...
	if (!(hu = kzalloc(sizeof(struct hci_stp), GFP_KERNEL))) {
	...
	*/
	...
	tasklet_init(&hci_tx_task_queue, hci_tx_tasklet,
                     (unsigned long) hu);
