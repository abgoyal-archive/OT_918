...
static int rds_cnt_get(struct rds_rx *rds_raw, int raw_size, int *cnt){
    ...
    *cnt = (raw_size - gap)/sizeof(rds_packet_struct);
    WCN_DBG(FM_INFO|D_RDS,"group cnt=%d\n", *cnt);
...
static int rds_group_get(uint16_t *dst, struct rds_rx *raw, int idx){
    ...
    WCN_DBG(FM_INFO|D_RDS,"BLOCK:%04x %04x %04x %04x, CRC:%04x\n", dst[0], dst[1], dst[2], dst[3], dst[4]);
...
static int rds_cbc_get(uint16_t cbc, enum RDS_BLK_T blk){
    ...
    WCN_DBG(FM_INFO|D_RDS,"group cbc=0x%04x\n", cbc);
    return ret;
}
...
static int rds_group_type_get(uint16_t crc, uint16_t blk, uint8_t *type, uint8_t *subtype){
    ...
    if(valid == TRUE){
        ...
        *subtype = (blk & 0x0800)>>11; //version code(1bit), 0=vesionA, 1=versionB
        ...
        WCN_DBG(FM_WARNING|D_RDS,"Block1 CRC err\n");
        return -ERR_RDS_CRC;
    }
    
    WCN_DBG(FM_INFO|D_RDS,"Type=%d, subtype:%s\n", (int)*type, *subtype ? "version B" : "version A");
    return ret;
}
...
static int rds_group_counter_add(uint8_t type, uint8_t subtype, struct rds_group_cnt *gc)
{
    ...
    gc->total++;
    WCN_DBG(FM_INFO|D_RDS,"group counter:%d\n", (int)gc->total);
    return 0;
}
...
static int rds_group_pi_get(uint16_t crc, uint16_t blk, uint16_t *pi, bool *dirty){
    ...
    if(valid == TRUE){
        ...
        WCN_DBG(FM_WARNING|D_RDS,"Block0 CRC err\n");
        return -ERR_RDS_CRC;
    }

    WCN_DBG(FM_INFO|D_RDS,"PI=0x%04x, %s\n", *pi, *dirty ? "new" : "old");
    return ret;
}
...
static int rds_group0_music_get(uint16_t blk, uint8_t *music, bool *dirty){
    ...
    WCN_DBG(FM_INFO|D_G0,"Music=%d, %s\n", (int)*music, *dirty ? "new" : "old");
    return ret;
}
...
static int rds_group2_rt_addr_get(uint16_t blk, uint8_t *addr){
    ...
    *addr = (uint8_t)blk & 0x0F;
    ...
    WCN_DBG(FM_INFO|D_G2,"addr=0x%02x\n", *addr);
    return ret;
}
...
static int rds_group2_txtAB_get(uint16_t blk, uint8_t *txtAB, bool *dirty){
    ...
    WCN_DBG(FM_INFO|D_G2,"txtAB=%d, %s\n", *txtAB, *dirty ? "new" : "old");
    return ret;
}
...
static int rds_group2_rt_get(uint16_t crc, uint8_t subtype, uint16_t blkC, uint16_t blkD, uint8_t addr, uint8_t *buf){
    ...
    if(addr > 0xFF){
        WCN_DBG(FM_ERR|D_RDS,"addr invalid(0x%02x)\n", addr);
        ret = -ERR_INVALID_PARA;
        return ret;
    }
    switch(subtype){
        case RDS_GRP_VER_A:
            ...
            if(valid == TRUE){
                ...
                WCN_DBG(FM_ERR|D_RDS,"rt crc check err\n");
                ret = -ERR_RDS_CRC;
            }
            break;
        case RDS_GRP_VER_B:
    ...
    WCN_DBG(FM_INFO|D_G2,"addr[%02x]:0x%02x 0x%02x 0x%02x 0x%02x\n", addr, buf[idx], buf[idx+1], buf[idx+2], buf[idx+3]);
    return ret;
}
...
static int rds_group2_rt_cmp(uint8_t addr, uint16_t cbc, uint8_t subtype, uint8_t *fresh, 
    ...
    WCN_DBG(FM_INFO|D_G2,"RT seg=%s\n", *valid == TRUE ? "TRUE" : "FALSE");
    WCN_DBG(FM_INFO|D_G2,"RT end=%s\n", *end == TRUE ? "TRUE" : "FALSE");
    WCN_DBG(FM_INFO|D_G2,"RT len=%d\n", *len);
    return ret;
}
...
static int rds_group2_rt_addr_bitmap_set(uint16_t *bitmap, int *bm_cnt, uint8_t addr){
    ...
    *bitmap |= (1<<addr); //set bitmap
    ...
    WCN_DBG(FM_NOTICE|D_G2,"RT bitmap=0x%04x, bmcnt=%d\n", *bitmap, *bm_cnt);
    return ret;
}
...
int MT6620_RDS_BlerCheck(struct fm *fm)
{
	...
	WCN_DBG(FM_DEBUG|D_BLKC,"+T:%d, MT6620_RDS_BlerCheck\n", jiffies_to_msecs(jiffies));
    ...
    if(pstRDSData->AF_Data.Addr_Cnt == 0xFF){
        ...
        if(pstRDSData->event_status != 0){
            ...
            WCN_DBG(FM_DEBUG|D_BLKC,"RDS_EVENT_AF, trigger read\n");
        }
	}
	...
	if((ret = MT6620_RDS_Get_GoodBlock_Counter(&GOOD_BLK_CNT)))
        return ret;
	WCN_DBG(FM_DEBUG|D_BLKC,"-T:%d, GOOD_BLK_CNT:%d\n", jiffies_to_msecs(jiffies), GOOD_BLK_CNT);
	...
	if((ret = MT6620_RDS_Reset_Block_Counter()))
        return ret;
	WCN_DBG(FM_DEBUG|D_BLKC,"BLER: TOTAL_CNT:%d BAD_BLK_CNT:%d, RDS_Sync_Cnt:%d\n", TOTAL_CNT, BAD_BLK_CNT, RDS_Sync_Cnt);
	...
	if((BAD_BLK_RATIO < MT6620_RDS_BLER_TH2)&&(RDS_Sync_Cnt > MT6620_RDS_BLER_C1)){
		...
		if(BAD_BLK_RATIO > MT6620_RDS_BLER_TH1){
			...
			if((RDS_Block_Reset_Cnt > MT6620_RDS_BLER_C2)||bRDS_FirstIn){
				...
				WCN_DBG(FM_DEBUG|D_BLKC,"RDS Reset, blk_cnt:%d, RDS_FirstIn:%d\n", RDS_Block_Reset_Cnt, bRDS_FirstIn);
			}else if(TOTAL_CNT > 12){
			    //LCH question 2, why 12???
				WCN_DBG(FM_DEBUG|D_BLKC,"RDS Block Reset: %x\n", RDS_Block_Reset_Cnt);
				if((ret = MT6620_RDS_Reset_Block()))
                    return ret;
			}
		}else{    
			RDS_Sync_Cnt++; //(60%-90%)	
			WCN_DBG(FM_DEBUG|D_BLKC,"RDS Sync Cnt: %d\n", RDS_Block_Reset_Cnt);	
			if(RDS_Block_Reset_Cnt > 1)
				RDS_Block_Reset_Cnt--;
...
static int MT6620_RDS_RetrieveGroup0(uint16_t *block_data, uint8_t SubType, RDSData_Struct *pstRDSData)
{
    ...
    if(valid == FALSE){
        WCN_DBG(FM_WARNING|D_G0,"Group0 BlockD crc err\n");
    	return -ERR_RDS_CRC;
    }
    
    ret = rds_group0_ta_get(block_data[1], &pstRDSData->RDSFlag.TA, &dirty);
    if(ret){
        WCN_DBG(FM_WARNING|D_G0,"get ta failed[ret=%d]\n", ret);
    }else if(dirty == TRUE){
    ...
    if(ret){
        WCN_DBG(FM_WARNING|D_G0,"get music failed[ret=%d]\n", ret);
    }else if(dirty == TRUE){
	...
	if(!SubType){
        ...
        if(valid == FALSE){
            WCN_DBG(FM_WARNING|D_G0,"Group0 BlockC crc err\n");
            return -ERR_RDS_CRC;
        }else{
			...
			if((AF_H > 224)&&(AF_H < 250)){
			    ...
			    WCN_DBG(FM_DEBUG|D_G0,"RetrieveGroup0 AF_H:%d, AF_L:%d\n", AF_H, AF_L);
				...
				if((AF_L < 205) && (AF_L > 0)){
					...
					WCN_DBG(FM_DEBUG|D_G0,"RetrieveGroup0 AF[0][0]:%d\n", pstRDSData->AF_Data.AF[0][0]);
					...
					WCN_DBG(FM_DEBUG|D_G0,"RetrieveGroup0 isAFNum_Get:%d, isMethod_A:%d\n", pstRDSData->AF_Data.isAFNum_Get, pstRDSData->AF_Data.isMethod_A);
					...
					if((pstRDSData->AF_Data.isAFNum_Get)&& (pstRDSData->AF_Data.AF_Num == 1)){
						...
						WCN_DBG(FM_DEBUG|D_G0,"RetrieveGroup0 RDS_EVENT_AF_LIST update\n");
				    }					
				}				
			}				
				...
				num = num>>1;				
                WCN_DBG(FM_DEBUG|D_G0,"RetrieveGroup0 +num:%d\n", num);
			    ...
			    for(indx = 1; indx < (num+1); indx++){
                    if((AF_H == (pstRDSData->AF_Data.AF[0][2*num-1]))&&(AF_L == (pstRDSData->AF_Data.AF[0][2*indx]))){
					    WCN_DBG(FM_ERR|D_G0|D_ALL,"RetrieveGroup0 AF same as indx:%d\n", indx);
						break;
						...
						WCN_DBG(FM_DEBUG|D_G0,"RetrieveGroup0 AF[0][%d]:%d, AF[0][%d]:%d\n", 
			    ...
			    if(num > 0){
			        if((pstRDSData->AF_Data.AF[0][num-1]) != 0){
						...
						WCN_DBG(FM_DEBUG|D_G0,"RetrieveGroup0 indx2:%d, num:%d\n", indx2, num);
						//compare AF buff0 and buff1 data:end						
						if(indx2 == num){
						    pstRDSData->AF_Data.Addr_Cnt = 0xFF;
						    pstRDSData->event_status |= RDS_EVENT_AF_LIST;
							WCN_DBG(FM_DEBUG|D_G0,"RetrieveGroup0 AF_Num:%d\n", pstRDSData->AF_Data.AF_Num);
						    for(indx = 0; indx < num; indx++){
								if((pstRDSData->AF_Data.AF[1][indx]) == 0){
    ...
    if((AF_H == (block_data[3])>>8)&&(AF_L == (block_data[3]&0xFF))){
	    if((!((pstRDSData->event_status)&RDS_EVENT_PROGRAMNAME))&&((PS_Num == 0)||(pstRDSData->PS_Data.Addr_Cnt))){
			...
			WCN_DBG(FM_DEBUG|D_G0,"RetrieveGroup0 PS second time, NUM:%x H:%x L:%x\n", 
			...
			WCN_DBG(FM_DEBUG|D_G0,"RetrieveGroup0, Addr_Cnt:%x\n", pstRDSData->PS_Data.Addr_Cnt);
		...
		WCN_DBG(FM_DEBUG|D_G0,"RetrieveGroup0 PS, NUM:%x H:%x L:%x\n", 
	...
	if((pstRDSData->event_status)&RDS_EVENT_PROGRAMNAME){
		...
		if(PS_Num == 0xFF){
			WCN_DBG(FM_ERR|D_G0|D_ALL,"RDS PS Canncel event 0x08");
			pstRDSData->event_status &= (~RDS_EVENT_PROGRAMNAME);
	    }
	} 
...
static int MT6620_RDS_RetrieveGroup2(uint16_t *block_data, uint8_t SubType, RDSData_Struct *pstRDSData)
{
	...
	WCN_DBG(FM_DEBUG|D_G2,"RT RetrieveGroup2 TextABFlag: %x --> %x\n", PreTextABFlag, pstRDSData->RDSFlag.Text_AB);
	...
	if(PreTextABFlag != pstRDSData->RDSFlag.Text_AB){ // Text A/B changed, clear old RT, and get new RT
		...
		if (pstRDSData->RT_Data.isRTDisplay == 0) {
			WCN_DBG(FM_WARNING|D_G2,"RT_Data.isRTDisplay == 0, and TextAB changed\n");
			...
			if (pstRDSData->event_status & RDS_EVENT_LAST_RADIOTEXT){
				...
				WCN_DBG(FM_WARNING|D_G2,"RT_Data.isRTDisplay = 1, no 0x0D case.\n");
        ...
        WCN_DBG(FM_INFO|D_G2,"RDS RT Get Len: [AddrMap=0x%x] [Length=0x%x]\n", pstRDSData->RT_Data.Addr_Cnt, pstRDSData->RT_Data.TextLength);
	...
	if(!SubType){
		...
		WCN_DBG(FM_NOTICE|D_G2,"%04x %04x %04x %04x %04x", block_data[0], block_data[1], block_data[2], block_data[3], block_data[4]);
		...
		//to get the 4bytes RadioText
	  	if(block_data[4]&(FM_RDS_GDBK_IND_C|FM_RDS_GDBK_IND_D)){
			...
			if(space == 4){
				...
				WCN_DBG(FM_DEBUG|D_G2,"RetrieveGroup2 RT addrcnt:%d, RT_Data.Addr_Cnt:%d\n", addrcnt, pstRDSData->RT_Data.Addr_Cnt);
			...
			WCN_DBG(FM_ERR|D_G2|D_ALL,"RT %04x %04x %04x %04x %04x CRC error.", block_data[0], block_data[1], block_data[2], block_data[3], block_data[4]);
	  	}
	  	for(indx = 0; indx < 4; indx++){
	    ...
	    //WCN_DBG(FM_DEBUG|D_MAIN,"RetrieveGroup2 Type B RT NUM: 0x%x Text: 0x%x", TextAddr, block_data[3]);
		WCN_DBG(FM_DEBUG|D_G2,"RT %04x %04x %04x %04x %04x", block_data[0], block_data[1], block_data[2], block_data[3], block_data[4]);
		...
		if(block_data[4]&FM_RDS_GDBK_IND_D){
			...
			if(space == 2){
			    addrcnt = pstRDSData->RT_Data.Addr_Cnt;
				pstRDSData->RT_Data.Addr_Cnt |= (1<<TextAddr);
				WCN_DBG(FM_DEBUG|D_G2,"RT RetrieveGroup2 RT B addrcnt: 0x%x, RT_Data.Addr_Cnt: 0x%x\n", addrcnt, pstRDSData->RT_Data.Addr_Cnt);
			...
			WCN_DBG(FM_DEBUG|D_G2,"RT %04x %04x %04x %04x %04x CRC error.", block_data[0], block_data[1], block_data[2], block_data[3], block_data[4]);
		}
    ...
    WCN_DBG(FM_NOTICE|D_G2,"RetrieveGroup2 RDS RT: Addr_Cnt: 0x%x Length: 0x%x addrcnt: 0x%x\n", pstRDSData->RT_Data.Addr_Cnt, pstRDSData->RT_Data.TextLength, addrcnt);
	...
	if(((((pstRDSData->RT_Data.Addr_Cnt)&addrcnt) == addrcnt)||((TextAddr == 0x0f) && (pstRDSData->RT_Data.Addr_Cnt == 0xffff)))){		
		...
		//pstRDSData->RT_Data.isRTDisplay = 1;
		pstRDSData->event_status |= RDS_EVENT_LAST_RADIOTEXT; 
		WCN_DBG(FM_DEBUG|D_G2,"RT RetrieveGroup2 isRTDisplay:%d\n", pstRDSData->RT_Data.isRTDisplay);
		space = 0;
		for(indx = 0; indx < 64; indx++){
		...
		if (pstRDSData->event_status & RDS_EVENT_LAST_RADIOTEXT){
			...
			WCN_DBG(FM_WARNING|D_G2,"RT_Data.isRTDisplay = 1\n");
			pstRDSData->RT_Data.isRTDisplay = 1;
		}
    }
    ...
    while(1){
        switch(rds_rt_state_get()){
            ...
            case RDS_RT_GETLEN:
                ...
                WCN_DBG(FM_NOTICE|D_G2,"Yes, get an RT! [len=%d]\n", rt_len);
...
static int MT6620_RDS_RetrieveGroup4(uint16_t *block_data, uint8_t SubType, RDSData_Struct *pstRDSData)
{
    ...
    uint32_t MJD, D1;
    int ret = 0;
	WCN_DBG(FM_DEBUG|D_G4,"RetrieveGroup4 %d\n", SubType);
    if(!SubType){
        //Type A
...
static int MT6620_RDS_RetrieveGroup14(uint16_t *block_data, uint8_t SubType, RDSData_Struct *pstRDSData)
{
    ...
    int ret = 0;
    WCN_DBG(FM_DEBUG|D_G14,"RetrieveGroup14 %d\n", SubType);
	...
	if((!SubType) && (block_data[4]&FM_RDS_GDBK_IND_C)){
			...
			if((AF_H > 223)&&(AF_H < 250)){
				...
				for(indx = 1; indx < (num+1); indx++){
		       		if((AF_H == (pstRDSData->AFON_Data.AF[0][2*indx-1]))&&(AF_L == (pstRDSData->AFON_Data.AF[0][2*indx]))){
					    WCN_DBG(FM_NOTICE|D_G14,"RetrieveGroup14 AFON same as indx:%d\n", indx);
						break;
		...
		WCN_DBG(FM_DEBUG|D_G14,"TA group14 typeB pstRDSData->RDSFlag.TP=%d pstRDSData->RDSFlag.TA=%d TP_ON=%d TA_ON=%d\n", pstRDSData->RDSFlag.TP, pstRDSData->RDSFlag.TA, TP_ON, TA_ON);
        if((!pstRDSData->RDSFlag.TP)&&(pstRDSData->RDSFlag.TA)&&TP_ON&&TA_ON){			
			...
			WCN_DBG(FM_NOTICE|D_G14,"TA set RDS_EVENT_TAON");
...
int MT6620_RDS_Eint_Handler(struct fm *fm, struct rds_rx *rds_raw, int rds_size)
{
    ...
    if(ret){
        WCN_DBG(FM_WARNING|D_RDS,"get cnt err[ret=%d]\n", ret);
        return ret;
    }
	...
	while(rds_cnt > 0){
        ...
        if(ret){
            WCN_DBG(FM_WARNING|D_RDS,"get group err[ret=%d]\n", ret);
            goto do_next;
	    }
	    
        ret = rds_group_type_get(block_data[4], block_data[1], &GroupType, &SubType);
		if(ret){
            WCN_DBG(FM_WARNING|D_RDS,"get group type err[ret=%d]\n", ret);
            goto do_next; 
			}			
        ...
        if(ret){
            WCN_DBG(FM_WARNING|D_RDS,"get group pi err[ret=%d]\n", ret);
            goto do_next;
        }else if(dirty == TRUE){
        ...
        if(ret){
            WCN_DBG(FM_WARNING|D_RDS,"get group pty err[ret=%d]\n", ret);
            goto do_next;
        }else if(dirty == TRUE){
        ...
        if(ret){
            WCN_DBG(FM_WARNING|D_RDS,"get group tp err[ret=%d]\n", ret);
            goto do_next;
        }else if(dirty == TRUE){
...
do_next:
        if(ret && (ret != -ERR_RDS_CRC)){
            WCN_DBG(FM_ERR|D_RDS,"parsing err[ret=%d]\n", ret);
            return ret;
        }
        rds_cnt--;
        i++;
